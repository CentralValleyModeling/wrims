...
C     Get the time interval
      IERR = 1
      CALL ZGINTL (INTL, CPATH(IBPART(5):IEPART(5)), NVALS, IERR)
      IF (IERR.EQ.0) THEN
C     Regular interval
      CALL ZBEGDT (JUL, INTL, IYR, IMON, IDAY, IBLOCK, IFLTAB(KVERNO))
      ELSE IF (IERR.EQ.1) THEN
C     Irregular interval
      CALL ZIRBEG (IFLTAB, JUL, CPATH(IBPART(5):IEPART(5)), IYR,
