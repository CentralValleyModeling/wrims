...
C-----1. READS INPUT FILES FROM THE NEW PICKWY PROGRAM (YEAR IS PART OF FILENAME)
      ...
      OPEN(1,FILE='DICU5.14',FORM='FORMATTED',STATUS='OLD')
      OPEN(2,FILE='DICU5.17',FORM='FORMATTED',STATUS='OLD')

      OPEN(3,FILE='IRREFF.DAT',FORM='FORMATTED',STATUS='OLD')
      OPEN(4,FILE='DIVFCTR.RMA',FORM='FORMATTED',STATUS='OLD')
      OPEN(7,FILE='DRNFCTR.RMA',FORM='FORMATTED',STATUS='OLD')
      OPEN(8,FILE='LEACHAPL.DAT',FORM='FORMATTED',STATUS='OLD')
      OPEN(9,FILE='LEACHDRN.DAT',FORM='FORMATTED',STATUS='OLD')

       OPEN(10,FILE='DICU5.12',FORM='FORMATTED',STATUS='OLD')
       OPEN(11,FILE='DICU5.27',FORM='FORMATTED',STATUS='OLD')

C*    MODIFY PROGRAM TO OUTPUT EITHER CL OR TDS CONCENTRATIONS --NM(5/7/91)

      IF (CONC.EQ.1) THEN
      OPEN(12,FILE='IDRNTDS.DAT',FORM='FORMATTED',STATUS='OLD')
      ENDIF

      IF (CONC.EQ.2) THEN
      OPEN(12,FILE='DRNCL.123',FORM='FORMATTED',STATUS='OLD')
      ENDIF
...
C@----MAX INCREASED FROM 10000 to 50000
 ...
 910     FORMAT(/A1)
         ...
         DO YR=IYR1,IYR2
            ...
            READ(2,('(I3,4X,I4,12F8.0)')) ISLDICU,IYR,(AW(ISLDICU,M,YR),M=1,12)
 ...
 990           FORMAT(' ERROR... Changes in the format of DICU output files'/
     ...
     &                '          to accomodate the changes')
               STOP
            ENDIF
            READ(1,('(I3,4X,I4,12F8.0)')) ISLDICU,IYR,(S(ISLDICU,M,YR),M=1,12)
            READ(10,('(I3,4X,I4,12F8.0)')) ISLDICU,IYR,(PREC(ISLDICU,M,YR),M=1,12)
            READ(11,('(I3,4X,I4,12F8.0)')) ISLDICU,IYR,(TCU(ISLDICU,M,YR),M=1,12)
...
C12A---Read in data to calculate new seepage component (drained seepage)
      ...
      open(100,file='subarea-info',form='formatted',status='old')
...
C-----READ THE LIST OF DWRDSM MODEL NODES
      ...
      OPEN(50,FILE="GEOM-NODES",FORM='FORMATTED',STATUS="OLD")
      READ(50,*) NUMBER
      DO 805 I=1,NUMBER
         READ (50,*) NODE(I)
 805  CONTINUE
            ...
            DO K=1,NUMBER
               ...
               IF(NDFLG(N))THEN
                  IF(N.NE.193)THEN
                     IF(IFLOW.EQ.1)THEN
                        WRITE(52,1910)N
 1910                   FORMAT('A=DICU-HIST+NODE  B=',I4,'  C=DRAIN-FLOW  D=01JAN1920  E=1MONTH  F=DWR-OSP'/
     &                       'CFS'/
     &                       'PER-AVER'/
     &                       '31OCT1921 2400')
                     ELSEIF(IFLOW.EQ.2)THEN
                        WRITE(52,1911)N
 1911                   FORMAT('A=DICU-HIST+NODE  B=',I4,'  C=DIV-FLOW  D=01JAN1920  E=1MONTH  F=DWR-OSP'/
     &                       'CFS'/
     &                       'PER-AVER'/
     &                       '31OCT1921 2400')
                     ELSEIF(IFLOW.EQ.3)THEN
                        WRITE(52,1912)N
 1912                   FORMAT('A=DICU-HIST+NODE  B=',I4,'  C=SEEP-FLOW  D=01JAN1920  E=1MONTH  F=DWR-OSP'/
     &                       'CFS'/
     &                       'PER-AVER'/
     &                       '31OCT1921 2400')
                     ENDIF
...
C--------------------Name for node 193 is changed to BBID (Byron Bethany)
                     IF(IFLOW.EQ.1)THEN
                        WRITE(52,1980)
 1980                   FORMAT('A=DICU-HIST+RSVR  B=','BBID','  C=DRAIN-FLOW  D=01JAN1920  E=1MONTH  F=DWR-OSP'/
     &                       'CFS'/
     &                       'PER-AVER'/
     &                       '31OCT1921 2400')
                     ELSEIF(IFLOW.EQ.2)THEN
                        WRITE(52,1981)
 1981                   FORMAT('A=DICU-HIST+RSVR  B=','BBID','  C=DIV-FLOW  D=01JAN1920  E=1MONTH  F=DWR-OSP'/
     &                       'CFS'/
     &                       'PER-AVER'/
     &                       '31OCT1921 2400')
                     ELSEIF(IFLOW.EQ.3)THEN
                        WRITE(52,1982)
 1982                   FORMAT('A=DICU-HIST+RSVR  B=','BBID','  C=SEEP-FLOW  D=01JAN1920  E=1MONTH  F=DWR-OSP'/
     &                       'CFS'/
     &                       'PER-AVER'/
     &                       '31OCT1921 2400')
                     ENDIF
