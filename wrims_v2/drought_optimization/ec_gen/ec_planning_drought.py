...
def ec_doc(ndo,startdt,enddt):
    ...
    dss_store_ts(mtzecfill,"timeseries/optimizer.dss",path1)
...
def gen_ec(argv=None,ndo=None):
    ...
    HISTRUN  = paradic["HISTRUN"]
    PLANNINGRUN  = paradic["PLANNINGRUN"]
    PREPRO_DIR = paradic["PREPRO_DIR"]
    DICU_DSSFILE = paradic["DICU_DSSFILE"]
    HIST_DIR = paradic["HIST_DIR"]
    HIST_GATE_FILE = paradic["HIST_GATE_FILE"]
    HIST_DSS_FILE = paradic["HIST_DSS_FILE"]
    Planning_Gate_File = paradic["Planning_Gate_File"]
    Update_DSS_File  = paradic["Update_DSS_File"]
    Planning_Dss_File = paradic["Planning_Dss_File"]
    Planning_ADICU_DssFile = paradic['Planning_ADICU_DssFile']
    End_Planning_Date = paradic["End_Planning_Date"]
    #DCU_values = paradic["DCU"].strip().split()
    cdecdss = os.path.join(PREPRO_DIR,"cdec.dss")
    astrodss = os.path.join(PREPRO_DIR,"astro-20years.dss")
    wkdss = paradic["Update_DSS_File"]    #"myworking.dss"
    YEAR = PLANNINGRUN
    REAL = PREPRO_DIR
    ...
    if Start_Planning_Date.find("-")>-1:
        datelst = Start_Planning_Date.split("-")
        sdatetime = datelst[1]+"/"+datelst[2]+"/"+datelst[0] +" 00:00"
        syear = int(datelst[0])  #integer
        smon = int(datelst[1])
    else:
...
def echo_outflow(echofile,alt):
    path = "/DSM2/NDO_BASE/FLOW//1DAY/%s/" % alt
