      SUBROUTINE ZCKPDD (IFLTAB, INSTR, NFIXED, ISTAT)
C
C
...
C     0 - Just check DSS file version number
...
C     4 - Update and fix file regardless of version (recommended for Unix)
C
...
C     Data Array      
      ...
      CALL ZGTREC (IFLTAB, ILBUFF, NTOT, IDADD, .FALSE.)
      NVAL = NTOT / 2
      IF (IFLTAB(KDSWAP).NE.0) CALL ZDSWAP(DBUFF, NTOT)
      DP = 0.0
      NCOUNT = 0
      LUPDATE = .FALSE.
      DO 150 J=1,NVAL
...
C     
      IF (LUPDATE) THEN
         NFIXED = NFIXED + 1
         IF (INSTR.GE.3) THEN
            CALL ZDSWAP(DBUFF, NTOT)
            CALL ZPTREC (IFLTAB, ILBUFF, NTOT, IDADD, .FALSE.)
             IF (MLEVEL.GE.3) WRITE (MUNIT,190) CPATH(1:NPATH)
 190  FORMAT (' -----DSS--- ZCKPDD;  Updated Precision for Record: ',A)
         ENDIF
      ENDIF
C
      ENDIF
...
C        Update the file version number to show that we have updated the file
