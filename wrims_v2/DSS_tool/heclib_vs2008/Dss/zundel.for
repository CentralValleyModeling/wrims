...
C
 ...
 20   FORMAT (T8,'-----DSS---Debug: Enter ZUNDEL')
C
C
C
      ISTAT = 0
...
C     Make sure the status flag is set to delete
 ...
 80   FORMAT (' -----DSS***ZUNDEL Unit',I5,':  Record Already Exists;',/
     * ' Pathname: ',A)
      ISTAT = 2
      GO TO 200
      ENDIF
...
C
 ...
 100  FORMAT (1X,'-----DSS---ZUNDEL Unit',I5,':  ',A)
C
C
      ELSE
C
C     That record was not found!
 ...
 160  FORMAT (' -----DSS***ZUNDEL Unit',I5,':  Unable to Find Record;',/
     * ' Pathname: ',A)
      ISTAT = 1
C
      ENDIF
...
C
 ...
 820  FORMAT (T8,'-----DSS---Debug:  Exit ZUNDEL')
*      CALL FLUSH(MUNIT)                                      Mu
C
      RETURN
C
