...
//			JList rowHeaderList = ((MonthlyTableModel.SingleMonthlyTable.SingleMonthlyTableModel)
...
//			setShowTotalColumn(((MonthlyTableFrame)_parentFrame).showTotalColumn());
	...
	public int setData(java.util.List dataSets, int firstMonth, boolean showCommas,
			boolean showTotalColumn, int decimalPlaces) {
		if (dataSets.size() == 0)
			return -1;
		...
		if (dataSet instanceof TimeSeriesContainer) {
			...
			if (((TimeSeriesContainer)dataSet).units.trim().equalsIgnoreCase("TAF")
					|| (showTotalColumn && ((TimeSeriesContainer)dataSet).units.trim().equalsIgnoreCase("CFS")))
				hasTotalColumn = true;
			else
				hasTotalColumn = false;
			((MonthlyTableModel) _tableModel).setData(dataSets, firstMonth, showCommas,
					hasTotalColumn, decimalPlaces);
		} else {
			if (dataSet instanceof TimeSeriesDataSet) {
				...
				setData(v, firstMonth, showCommas, showTotalColumn, decimalPlaces);
			} else {
	...
	public void setupModel() {
		...
		while (dataSets.hasMoreElements()) {
			MonthlyTableModel.SingleMonthlyTable table
				= (MonthlyTableModel.SingleMonthlyTable)dataSets.nextElement();
			// The next line is necessary, but if you query getRowHeight() after it, it says 16.  Why?
			setRowHeight(i, table.getRowCount() * table.getRowHeight()
				+ table.getTableHeader().getPreferredSize().height + 3); // the 3 is correct, but why?
			++i;
		}
	}
	...
	public void setShowCommas(boolean showCommas) {
		if (_tableModel != null)  _tableModel.setShowCommas(showCommas);
	}
	...
	public void setPrecision(int precision) {
		if (_tableModel != null) _tableModel.setPrecision(precision);
	}
	...
	public String getToolTipText(MouseEvent event) {
		...
		if (row > 0 && column > 1)
			return ((MonthlyTableModel) _tableModel).getToolTipText(row, column);
		else
	...
	public int addData(TimeSeriesContainer timeSeriesContainer, int firstMonth,
			boolean showCommas, boolean showTotalColumn, int decimalPlaces) {
		int status = -1;
		if (_tableModel == null) _tableModel = new MonthlyTableModel();  // CB TO DO: need??????
		if (_tableModel instanceof MonthlyTableModel) {
			Vector v = new Vector();
			v.add(timeSeriesContainer);
			status = _tableModel.addData(v, firstMonth, showCommas, showTotalColumn, decimalPlaces);
		}
		return status;
	}
	...
	public boolean print() {
		if (getModel() instanceof MonthlyTableModel) {
			for (int i = 0; i < ((MonthlyTableModel)getModel()).getRowCount(); ++i) {
				if (((MonthlyTableModel)getModel()).getValueAt(i, 0) instanceof
						MonthlyTableModel.SingleMonthlyTable) {
					try {
						((MonthlyTableModel.SingleMonthlyTable)((MonthlyTableModel)getModel())
								.getValueAt(i, 0)).print(PrintMode.FIT_WIDTH, null, null, false,
										null, false);
