...
/**
 * Frame for showing and specifying mathematical equation of the DeriverTimeSeries.  Allows addition or
 * deletion of new rows (terms).
 *
 * @author Clay Booher
 */
public class DashboardVariableWindow extends JFrame implements ListSelectionListener {
...
//	AllLayerTableModel _allLayerTableModel;
	 ...
	 */
//	public DashboardVariableWindow(String commonName, String variableName, FilterPanel panel) {
	DashboardVariableWindow(String commonName, DerivedTimeSeries dts, FilterPanel panel) {
...
//		buildWindow(commonName, variableName, panel);
	 ...
	 */
	public void buildWindow(String commonName, DerivedTimeSeries dts, FilterPanel panel) {
		buildMenu();
...
//		TableSorter sorter = new TableSorter(_dtsTable.getTable().getModel());  //CB TODO ? if need, then put into DTSTable class
...
//		setTableColumnWidths(_dtsTable.getTable());
		...
		lsm.addListSelectionListener(this); //listen for row selection events in _allLayerTable
...
//	}
	 ...
	 * @return
	 */
/*	public AllLayerTableModel getAllLayerTableModel() {
		return _allLayerTableModel;
	} */
...
/*	void writePreferences() {
		writeColumnWidthPreferences(_allLayerTable);
	} */
...
/*	public void refreshData() {
		//re-set the table models
		_allLayerTableModel = new AllLayerTableModel();
		TableSorter sorter = new TableSorter(_allLayerTableModel); //CB added TableSorter
		_allLayerTable.setModel(sorter);
		setTableColumnWidths(_allLayerTable);
	} */

	/**
	 *
	 * @param e   the ListSelectionEvent which is only used to quickly return if the value is being
	 *            adjusted.
	 */
	public void valueChanged(ListSelectionEvent e) {
		if (!e.getValueIsAdjusting()) { //if not currently being adjusted
/*			if (_dtsTable.getTable().getSelectedRowCount() > 0) {
				int[] rowsSelected = _dtsTable.getTable().getSelectedRows();
