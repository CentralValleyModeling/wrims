      SUBROUTINE ZCKPNB (IFLTAB, NERROR)
C
...
C
 ...
 20   FORMAT (T8,'-----DSS---Debug:  Enter ZCKPNB;  Unit:',I5)
C
...
C     Should not get here
      WRITE (MUNIT,*)' IFLTAB CORRUPT IN ZCKPNB, KTABLE DOES NOT MATCH'
      CALL ZABORT (IFLTAB, 100, 'ZCKPNB', 0, IFLTAB(KTABLE), ' ')
      RETURN
      ENDIF
C
C
...
C     Get the information block
      JSIZE = NINFO+NPPWRD
      CALL ZGTREC (IFLTAB, INFO, JSIZE, JADD, .FALSE.)
C
...
C
 ...
 810  FORMAT (/,' *** ZCKPNB:  Inconsistency in the number of records',
...
C
 ...
 890  FORMAT (T8,'-----DSS---Debug:  Exit ZCKPNB;   Unit:',I5)
C
      RETURN
C
      END
