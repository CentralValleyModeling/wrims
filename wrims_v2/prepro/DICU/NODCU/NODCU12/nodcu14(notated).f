...
Ccccccccccccccccccccccccccccccccccccccccccccccccccccc      READ(*,801)ANSWER
...
Cc            READ(2,('(I3,4X,I4,12F8.0)')) ISLDICU,IYR,(AW(ISLDICU,M,YR),M=1,12)
ccc	AW# applied water required before island irrigation efficiency factored in
...
ccc	PREC# precipitation
            READ(10,980) ISLDICU,IYR,(PREC(ISLDICU,M,YR),M=1,12)
ccc	TCU# Total cunsumptive use for each island
...
ccc	IIE# irrigation efficiency
      READ(3,'(f4.2)')(IIE(I),I=1,MAXISL)
ccc	LRA# leach water applied
      READ(8,15)((LRA(I,M),M=1,12),I=1,MAXISL)
ccc	LRD# leach water drainage
...
ccc	FI# DIVERSION ALLOCATION FACTOR
ccc	FD# DRAINAGE ALLOCATION FACTOR AW
...
C      WRITE(20,21) I,N,FI(N,I)
    ...
    6 CONTINUE
      IPICK=0
ccc	rad to the end of file FD# drainage allocation factors
...
C      WRITE(20,21) I,N,FD(N,I)
    ...
    8 CONTINUE
      READ(12,19)
ccc	DS# drainage concentrations
...
ccc		RO# = Prec - ET but if < 0 then set it equal to 0
...
C--------
CNM1-----
Cc--------ID1 = the irrigation diversion component of the total diversion from the channel
Cc--------ID2 = the seepage component of the total diversion from the channel
CNM2
...
C..
         DO 500 I=1,MAXISL
            DO 400 M=1,12
ccc		ID# irrigation diversion = seepage + AW/0.70 + leach water app. 
ccc			The AW/0.70 is due to inefficiency, i.e. W(used)/W(appl.)=70% therefore
ccc			W(appl.)=W(used)/0.70 = AW/0.70
...
ccc		D# irrigation drain = 0.428*AW + leach water drain + RO
ccc		      The 0.428*AW comes from W(appl.)-W(used)=(1/eff-1)W(used)=(1/0.70-1)AW=0.428AW
               D(I,M)=( AW(I,M,YR) * ((1.-IIE(I))/IIE(I)) ) + LRD(I,M) + RO(I,M) + drnseep(i)
ccc		D2# drainage for Ag Drain Model? = 0.428*AW + leach water drain
...
C..
         DO 550 I=1,MAXISL
ccc		TAID# total irrigation diversion for each island for the year
            TAID(I)=0.
ccc		TAD# total irrigation drain for each island for the year
            TAD(I)=0.
            DO 525 M=1,12
               TAID(I)=TAID(I) + ID(I,M)
               TAD(I)=TAD(I) + D(I,M)
 525        CONTINUE
ccc		TADID# total diversion for delta for year
            TADID=TADID + TAID(I)
ccc		TADD# total drain for delta for year
            TADD=TADD + TAD(I)
 550     CONTINUE
ccc		CDDNET# net channel diversion for delta for year = div - drn
         CDDNET=TADID-TADD
C..
...
C..
         DO 600 M=1,12
            ...
            TMDD(M)=0.
            DO 575 I=1,MAXISL
ccc		TMDID# total irrigation diversion for month (all islands)
               TMDID(M)=TMDID(M) + ID(I,M)
ccc		TMDD# total irrigation drain for month (all islands)
               TMDD(M)=TMDD(M) + D(I,M)
 575        CONTINUE
ccc		CDNET# net channel depletion each month
...
ccc	NIDSUB# diversion (node, month) = (div factor (node, island)/ 100)* diversion(island,month)
...
ccc	NDSUB# drain (node, month) = (drain factor (node, month)/ 100)* drain (island, month)
                  NDSUB(N,M)=FD(N,I)/100. * D(I,M)
C..---------------
ccc	NID# total diversion for the node (different islands contribute to a particular node)
...
C...
C...
ccc	All other nodes get assigned a concentration from the island
...
C..
         DO 785 M=1,12
ccc	NTMDID# total diversion for all nodes per month
            NTMDID(M)=0.
ccc	NTMDD# total drain for all nodes per month
            NTMDD(M)=0.
            DO 775 N=1,MAXNODE
               NTMDID(M)=NTMDID(M) + NID(N,M)
               NTMDD(M)=NTMDD(M) + ND(N,M)
 775        CONTINUE
ccc	NCDNET# channel depletion for each month (should equal CDNET(m)?)
...
C..
         DO 900 N=1,MAXNODE
            DO 800 M=1,12
ccc	if mth = Nov or mth = Apr or mth = Jun or mth = Sep
...
Cc       write(*,*) "???? WARNING: Hard wiring to write dss data till only sep 1997), NM (12/1/97)"
Cc                        endif

Cc       if(yr.eq.98.and.m.gt.1) go to 9999
