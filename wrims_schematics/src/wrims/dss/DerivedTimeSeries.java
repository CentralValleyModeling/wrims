...
//	_userPrefs = Preferences.userNodeForPackage(getClass());  // CB added
	private static Preferences _userPrefs; //= Preferences.userNodeForPackage(getClass());
...
//					dashKeys.add(keys[i]);
			...
			for (int i = 0; i < parts.length; ++i) {
				if (i % ROW_SIZE == DTSTable.OPERATOR_INDEX) {
					...
					if (parts[i].trim().length() == 1
							&& (parts[i].trim().charAt(0) != '1'
								|| parts[i].trim().charAt(0) != '2')) {
	...
	void writePreferences() {
		...
		while (rows.hasMoreElements()) {
			...
			for (int i = 0; i < row.length; ++i) {  //CB no checks here - leave it up to user
				if (row[i] == null)
					value.append(" ");
				else
					value.append(row[i]);
...
//		System.out.println(value.toString());
		_userPrefs.put(FilterPanel.DASHBOARD_VARIABLE + _name, value.toString());
	}
...
//CB			_name = name.toUpperCase().trim();
	...
	boolean addRow(String[] row) {
		if (row.length != ROW_SIZE) {
			JOptionPane.showMessageDialog(null,"Row must be sam size as number of table columns",
