...
C              This should force us to mode 2
               CALL ZMULTU (IFLTAB, .TRUE., .FALSE.)                    Md
               CALL ZMULTU (IFLTAB, .FALSE., .FALSE.)                   Md
               IF (IFLTAB(KMEMORY).EQ.1) THEN                           Md
               CALL ZRMEM (IHANDL, IREC, IBUFF(1,JBUFF), NBSIZE, ISTAT) Md
               ELSE                                                     Md
               CALL ZRREC (IHANDL, IREC, IBUFF(1,JBUFF), NBSIZE,        Md
     *                     IFLTAB(KSWAP), ISTAT, JSTAT)                 Md
               ENDIF                                                    Md
            ENDIF                                                       Md
         ENDIF                                                          Md
...
C              Three words (12 bytes) are used in locking
               ...
               CALL seekf (IHANDL, 0, IOFSET, IPOS, JSTAT)              Md
