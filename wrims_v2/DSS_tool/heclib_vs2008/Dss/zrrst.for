     ...
     * KBASE,  ! dimenson of height & flow arrays (must be at least nbase)
     ...
     * FLOW,   ! flows
       ...
       ! shift info !
       !------------!
     * KSHIFT, ! dimension of arrays related to shifts (must be at least nshift)
...
c
     ...
     * ISTAT, NPATH, NVALS, ITYPE, NGBFU, NCHEAD, ICHEAD(KCHEAD),
   ...
   10 FORMAT (T5, '----- Enter ZRRST -----', /,
     ...
     * T11, 'Pathname: ', A)
      !-----------------------------------------------------------------!
      ! determine whether the record exists and is of the expected type !
      !-----------------------------------------------------------------!
      CALL ZDTYPE(IFLTAB, CPATH, NVALS, LFOUND, CTEMP1, ITYPE)
      IF (.NOT. LFOUND) GO TO 9990
      IF (ITYPE .NE. 560) GO TO 9980
 ...
 9980 CONTINUE
      IF (MLEVEL .GT. 1) WRITE (MUNIT, 9981) ITYPE, 560, CPATH(1:NPATH),
     * CPATH(1:NPATH)
