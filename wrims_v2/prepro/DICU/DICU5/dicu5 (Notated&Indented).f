...
ccc	  calculate precip
          DO 220 J=1,12
ccc	  monthly precip (J) for the current crop (KRP) = monthly precip * adj.factor {usually 1}
...
ccc	crop change in soil moist. = min(max(precip-crop cu,lower soilmoist - init.soilmoist.)
ccc						,upper soilmoist limit - init.soilmoist.)
...
ccc	crop change in soil moist. = - min(init.soilmoist. - lower soilmoist,max(crop cu - precip
ccc						,init.soilmoist. - upper soilmoist limit))
...
ccc	histCU_Ppt = histCU_Ppt + CU_UrbLawnPpt
            HCUP(J)=HCUP(J)+CUPLN(J)
ccc	CU_UrbLawnSeep = TotCU_UrbLawnPpt - CU_UrbLawnPpt
            CUSLN(J)=TCUPLN(J)-CUPLN(J)
ccc	histCU_Seep = histCU_Seep + CU_UrbLawnSeep
