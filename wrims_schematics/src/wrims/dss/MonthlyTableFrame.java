...
public class MonthlyTableFrame extends JFrame implements ActionListener {
	...
	public MonthlyTableFrame(Outputer parent) {
		...
		JComboBox _timeZoneBox = new JComboBox(precisionLevels); // for users to set timezone??????
...
//		_table = new MonthlyTable(this);
	...
	private void buildMenu() {
		...
		if (_parent instanceof DssViewer && ((DssViewer)_parent).isTAFSelected()) { //No average or total column when selected units are CFS
//			_showAverageColumn = false; // need to set in other places in class as well
			_viewTotalColumn = new JCheckBoxMenuItem("Average Column", _showTotalColumn);
	...
	public int setData(List dataSets, int firstMonth) {
		...
		//CB - cannot use units here due to more than one data set.  It is handled later
		if (_parent instanceof DssViewer && !((DssViewer)_parent).isTAFSelected())
			showTotalColumn = false;
		int status = _table.setData(dataSets, firstMonth, _showCommas, showTotalColumn,
			_decimalPlaces);
		if (status == 0) {
			showTable();
		}
	...
	public int setData(TimeSeriesContainer timeSeriesContainer, int firstMonth) {
		...
		if (_parent instanceof DssViewer && (!((DssViewer)_parent).isTAFSelected()
			...
			showTotalColumn = false;
		int status = _table.setData(timeSeriesContainer, firstMonth, _showCommas,
	...
	public int addData(DataContainer dc, int firstMonth) {
		...
		if (dc instanceof TimeSeriesContainer) {
			...
			//CB added - No total column when selected units are CFS
			if (_parent instanceof DssViewer && (!((DssViewer)_parent).isTAFSelected()
					|| !((TimeSeriesContainer)dc).units.trim().equalsIgnoreCase("TAF")))
				showTotalColumn = false;
			status = _table.addData(tsc, firstMonth, _showCommas, showTotalColumn,
				_decimalPlaces);	//CB altered
		}
		return status;
	}
	...
	/**
	 * CB added.
	 * Changes the global property if different and depending on selected units in DssViewer.
	 * Units of each data set must be checked elsewhere.
	...
	void setShowTotalColumn(boolean showTotalColumn) {
		...
		if (showTotalColumn != _showTotalColumn) {
			...
			if (_viewTotalColumn != null) {
				...
				validate();
				repaint();
				firePropertyChange(SHOW_TOTAL_COLUMN_PROPERTY, !_showTotalColumn, _showTotalColumn);
			}
		}
	}
	...
	void setDecimalPlaces(int decimalPlaces) {
		if (decimalPlaces != _decimalPlaces) {
			...
			if (menuitem != null) { // then all gui components are null; DecimalPlaces_Action cannot be called
				menuitem.setSelected(true);
				DecimalPlaces_Action(_decimalPlaces);
			} else {
				validate();
				repaint();
				firePropertyChange(DECIMAL_PLACES_PROPERTY, oldPlaces, _decimalPlaces);
			}
		}
	}
...
//			saveAs();
			...
			String title = _parent.getPlotTitle(exceedence, annualTot, monthlyAvg);
	...
	protected void Exit_Action() {
		updateEdits();
/*		if (_table.hasDataChanged() == true) {
			int n = JOptionPane.showConfirmDialog(this,
					"Save changes to data set?", "Save Edited Changes", 1);
			if (n == 0) {
				int errors = save();
				if (errors != 0)
					return;
	...
	public int updateContainers() {
		updateEdits();
/* CB		int[] firstError = new int[3];
		int numberErrors = _table.updateContainers(firstError);
		if (numberErrors > 0) {
			StringBuffer message = new StringBuffer();
			if (numberErrors > 1)
				message.append(numberErrors + " errors found.  First Error:\n");
			if (firstError[2] == 0)
				message.append("Invalid date / time at ordinate "
						+ (firstError[0] + 1));
			else {
				...
				message.append("\n    Date / time at ordinate "
						+ (firstError[0] + 1) + " is " + time);
			}
...
/*	public void saveAs() {
		int errors = updateContainers();
		if (errors == 0)
			_table.saveAs(_parent);
		validate();
	} */
...
/*	public int save() {
	...
	private void DecimalPlaces_Action(int numberDecimals) {
		...
		if (oldPlaces != numberDecimals) {
			...
			validate();
			repaint();
			firePropertyChange(DECIMAL_PLACES_PROPERTY, oldPlaces, numberDecimals);
		}
	}
	...
	public String getHTMLExportString() {
		updateContainers();
		if (_showCommas)
			_table.setShowCommas(false); // temporarily change _table's _showCommas to false
...
//			_table.plot(this);
	...
	void plot(String title, boolean isExceedence, boolean isAnnualTotal, boolean isMonthlyTotal) {
		int numberErrors = updateContainers();
		if (numberErrors == 0)
