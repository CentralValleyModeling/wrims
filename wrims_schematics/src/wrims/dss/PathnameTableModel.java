...
public class PathnameTableModel extends AbstractTableModel {
    ...
    private String[] _columnNames = { "Number","A part", "B part", "C part", "D part", "E part",
    	"F part" };
...
//    private Hashtable _matchColumns = new Hashtable();
    ...
    /**
     * CB added to help with reservoir inflow and outflow dss display without filtering on them.
     */
    public Object clone() {
    	PathnameTableModel newPTM = new PathnameTableModel(_pathnameParts, _condensedList);
    	newPTM.setData();
...
//    	newPTM.initVisibleRows();
    ...
    /**
     * CB added for obtaining paths for variable names passed to it (when the table is not
     * updated first (like when a reservoir node is right-clicked and Inflow is selected, we do
     * not want to filter the table on the inflow arc name).
     * @return
     */
    public Vector<String> getMatchingPaths(Vector<String> names) {
    	...
    	while (namesEnum.hasMoreElements()) {
	    	...
	    	for (int row = 0; row < getRowCount(); ++row) {
	    		String name2 = (String)getValueAt(row, 2); // hard coded column index for now
	    		if (name.equals((name2)))  //changed to equals to speed up and i think it is OK
    ...
    public int showFilteredRows(String filter, boolean useRegex) {
		...
		if (DEBUG) System.out.println("useRegex: "+useRegex);
		if (DEBUG) System.out.println("filter:["+search+"]");
        ...
        for (i=1; i<7; i++) {
		    ...
		    if (DEBUG) System.out.println(i+":"+parts[i]);
        ...
        for (i=0; i<_rowsVisible.length; i++) {
            ...
            for (j=1; j<7; j++) {
		        ...
		        //if (DEBUG) System.out.println("i:j"+i+":"+j+" -> "+pattern[j].matcher((String)_data[i][j]).matches());
            }
    ...
    public boolean isValidBParts(Vector bparts) {
        ...
        if (DEBUG) System.out.println( "bparts "+bparts);
    ...
    private void setData() {
        ...
        for (int i = 0; i < rows; i++) {
            for (int j=0; j< _columnNames.length; j++) {
                if (j == 0) d[i][j] = i+1;
                else d[i][j] = _pathnameParts[i][j-1];
                b[i][j] = false;
            }
        }
        _data = d;
	...
	public boolean isCellColored(int row, int col) {
		return _match[getNumber(row,_rowsVisible)][getNumber(col, _columnsVisible)];
	}
    ...
    public Object getValueAt(int row, int col) {
        return _data[getNumber(row,_rowsVisible)][getNumber(col, _columnsVisible)];
    }
...
//CB to speed up        String p[] = _pathnameParts[getNumber(row,_rowsVisible)];
    	int index = getNumber(row,_rowsVisible);    //CB to speed up
//CB to speed up         String p[] = _pathnameParts[getNumber(row,_rowsVisible)];
    	//CB to speed up        String path = "/"+p[0]+"/"+p[1]+"/"+p[2]+"//"+p[4]+"/"+p[5]+"/";
