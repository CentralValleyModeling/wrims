...
nsta=len(BList_mdl)
#
from com.sun.java.swing import JOptionPane
...
def plotStation(standx) :
  ...
  for data_type in ['inst', 'resid']:
    ...
    if len(gso) == 0:
      JOptionPane.showMessageDialog(None,  "Couldn't find observed data for " + 
				    repr(sta_obs))
      break
    if f_obs != '':
      gso.filterBy(f_obs)
      if len(gso) == 0:
        JOptionPane.showMessageDialog(None,  "Couldn't find observed data for " + 
				      repr(sta_obs) + repr(f_obs))
        break
    if len(gso) > 1:
      JOptionPane.showMessageDialog(None,  "Too many observed paths" + repr(gso))
      break
    ref_obs = DataReference.create(gso[0],tw)
    if ref_obs==None:
      JOptionPane.showMessageDialog(None,  "No Observed data for time window "+repr(tw))
      break
    if data_type=='resid':
    ...
    crv_mdl.setForegroundColor(DBlue)
    crv_mdl_good.setForegroundColor(Blue)
