      SUBROUTINE ZMULTU ( IFLTAB, LOCK, LFLUSH)
C
...
C
      IUNIT = IFLTAB(KUNIT)
      IF (MLEVEL.GE.10) WRITE (MUNIT, 20) IUNIT, IFLTAB(KMULT),
     * IFLTAB(KLOCK), LOCK, LFLUSH
 20   FORMAT (T5,'----DSS--- DEBUG:  Enter ZMULTU;  Unit:',I5,/,
...
C        Exclusive lock - lock the lock record
         ISIZE = IWORD(2) * 3
         CALL lockdss (IFLTAB(KHANDL), 2, IFLTAB(KLOCKB), ISIZE, IERR)
         IF (IERR.NE.0) THEN
            WRITE ( MUNIT, 30) IUNIT
 30         FORMAT (/,' ----- DSS ERROR ----*',/
     *       ' Unable to lock file for Exclusive access.   Unit: ',I5)
            CALL ZABORT (IFLTAB, 210, 'ZMULTU', IUNIT, IFLTAB(KLOCK),
     *       'Can not EXCLUSIVELY lock file')
            RETURN
         ENDIF
      ELSE
C
      CALL lockdss (IFLTAB(KHANDL), 2, IFLTAB(KLOCKB), IWORD(2), IERR)
      ENDIF
C
C
...
C           On MS Windows, this lock will wait up to 10 seconds
            CALL lockdss (IFLTAB(KHANDL), 1,IFLTAB(KLOCKB),
     *                     IWORD(2), JERR)
            IF (JERR.EQ.0) THEN
...
C              Write a message every 30 seconds
               J =  MOD (I, 3)
               IF ((J.EQ.1).AND.(MLEVEL.GE.2)) WRITE (MUNIT, 40)
...
C     call oldLockDSS (IFLTAB(KHANDL), 1,IFLTAB(KLOCKB),IWORD(2), JERR) u
      IFLTAB(KLOCK) = 2
      IF (MLEVEL.GE.10) WRITE (MUNIT,*)'---DSS:  File Locked'
C
C
...
C     phyical read (not just a buffer in memory)!!
      IF (LFLUSH) THEN
         IF (MLEVEL.GE.10) WRITE (MUNIT,*)'---DSS:  ZMULTU:  Read Perm'
...
C     First dump all buffers to disk
      IF (LFLUSH) THEN
         IF (MLEVEL.GE.10) WRITE (MUNIT,*)'---DSS:  ZMULTU:  Dump Buffs'
...
C
      ENDIF
C
      IF (MLEVEL.GE.10) WRITE (MUNIT, 120) IUNIT, IFLTAB(KMULT),
     * IFLTAB(KLOCK), LOCK
 120  FORMAT (T5,'----DSS--- DEBUG:  Exit ZMULTU;  Unit:',I5,/,
