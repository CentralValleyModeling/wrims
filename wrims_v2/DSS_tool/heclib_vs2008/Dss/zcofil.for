...
C
      ...
      LOGICAL LRCDEL, LRETAG, LFOUND, LPROT2, LTSCOPY
      CHARACTER CPATH*400
C
C
      INCLUDE 'zdsskz.h'
...
C
      INCLUDE 'zdssnz.h'
C
      COMMON /ZCOPYFL/ NUMB_RECS, NUMB_PATHS, CUR_REC, CUR_PATH,
     *                 INTERRUPT
      INTEGER NUMB_RECS, NUMB_PATHS, CUR_REC, CUR_PATH, INTERRUPT
C
C
C
C     The follow is if another thread interrupts us to stop the process
 ...
 20   FORMAT (T6,'-----DSS---Debug:  Enter ZCOFIL;  Unit:',I5)
C
...
C     call ZRRTSX, then ZSRTSX via ZCOREC
      LTSCOPY = .FALSE.
...
C     swap miss-match, use ZCOREC also
...
C     Check for addresses out of range.
 ...
 50   FORMAT(/,' -----DSS*** ZCOFIL;  Error: ',A,' out of Range',
...
C
 ...
 820  FORMAT (T6,'-----DSS---Debug:  Exit ZCOFIL')
      CALL FLUSH(MUNIT)                                                 Mu
C
      RETURN
C
C
 ...
 850  FORMAT (' ----- DSS --- ZCOFIL:  Copy interrupted.')
      GO TO 800
C
C
 900  CONTINUE
 ...
 901  FORMAT (/,' ----- DSS --- ZCOFIL:  ERROR;  Buffer size(s) not',
     ...
     * ';   Size Required:',I5,/)
      CALL ZABORT (IFTOLD, 331, 'ZCOFIL ', KBUFF, NBSIZE,
...
C
 930  CONTINUE
      WRITE (MUNIT, 931)
 931  FORMAT (/' -----DSS---ZCOFIL:  ERROR;  Files are of different',
...
C
 ...
 941  FORMAT (/,' *****DSS*** ZCOFIL:  ERROR  - UNABLE TO ',
