...
C
 ...
 20   FORMAT (T6,'-----DSS---Debug:  Enter ZPLIST;  Unit:',I5)
C
C
...
C        If this is the first entrance, set any selective catalog items
         ...
         IF (NINST.GT.0) THEN
            ...
            ELSE IF (CINST(1:2).EQ.'S=') THEN
               ISELCA = 1
            ELSE 
               CALL ZSETCA (CINST(1:NINST), LSELCA)
               IF (LSELCA) THEN
                  ISELCA = 2
                ENDIF				              
            ENDIF         
...
C
 ...
 920  FORMAT(' -----DSS---Error:  File error detected in ZPLIST')
C      CALL ZERROR (IFLTAB, 100, 'ZPLIST', 0, IFLTAB(KTABLE), ' ', 0,
C     * ' ', 0)
      IF (ISTAT.EQ.-2) THEN
      IF (MLEVEL.GE.1) WRITE(MUNIT,930) IADD
 930  FORMAT(' ZPLIST Error: File address out of range:',I10)
      ELSE IF (ISTAT.EQ.-3) THEN
	IF (MLEVEL.GE.1) WRITE(MUNIT,930) IADD
      ELSE IF (ISTAT.EQ.-4) THEN
      IF (MLEVEL.GE.1) WRITE(MUNIT,940) IPBIN(JPBIN)
 940  FORMAT(' ZPLIST Error: Pathname status out of range:',I10)
      ENDIF
