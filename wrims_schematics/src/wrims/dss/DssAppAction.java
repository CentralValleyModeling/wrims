...
//CB	private FilterPanel getFP() { //CB changed name to match AppAction's to reduce errors and confusion
	private FilterPanel getFilterPanel() {
		return _mp.getFilterPanel();
	}

//CB	private MessagePanel getMP() { //CB changed name to match AppAction's to reduce errors and confusion
	...
	// ==============================================================
	// ================= FilterPanel actions ========================
	public AppAction FilterAction = new AppAction("Filter", _mp, "Filter specified A-F parts") {
	...
	/*
	 * AppAction RetrieveAction = new AppAction("Retrieve", _mp, "Retrieve selected records") {
	 * public void actionPerformed(ActionEvent e) { getFP().retrieve("All");
	 * AppAction.updateAllActions(); } public boolean canAct() { return super.canAct() &&
	 * getFP().isFilterActive() && getFP()._table.getSelectedRowCount() > 0; } };
	 */

	//CB TODO create abstact classes for the next several "output" type actions and have them use the two canAct() methods "in common"
	...
	AppAction MonthlyAction = new AppAction(DssViewer.MONTHLY_STRING, _mp,
		...
		public boolean canAct() { //CB TODO need to add isCompActive() elsewhere!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//CB			return super.canAct() && getFP().isFilterActive() && getFP().rowsSelected();
			Object filterPanel = getFilterPanel();
			Object messagePanel = getMessagePanel();
			return super.canAct() && getMessagePanel().isCompActive()
//				&& !_mp.isSelectedFilesThreadAlive() && !_mp.isUnselectedFilesThreadAlive()
				&& getFilterPanel().isFilterActive() && (getFilterPanel().rowsSelected()
					|| getFilterPanel().areDashboardRowsSelected()); //CB altered
		}
	};

	AppAction TableAction = new AppAction(DssViewer.TABLE_STRING, _mp, "Display table of selected pathnames") {
		public void actionPerformed(ActionEvent e) {
//			System.out.println("in actionPerformed of AppAction TableAction"); //CB debugging
...
//CB			return super.canAct() && getFP().isFilterActive() && getFP().rowsSelected();
			return super.canAct()  && getMessagePanel().isCompActive()
				&& !_mp.isSelectedFilesThreadAlive() && !_mp.isUnselectedFilesThreadAlive()			
				&& getFilterPanel().isFilterActive() && (getFilterPanel().rowsSelected()
					|| getFilterPanel().areDashboardRowsSelected()); //CB
		}
	};

	AppAction PlotAction = new AppAction(DssViewer.PLOT_STRING, _mp, "Display plot of selected pathnames") {
		public void actionPerformed(ActionEvent e) {
...
//CB			return super.canAct() && getFP().isFilterActive() && getFP().rowsSelected();
	...
	AppAction ExceedenceAction = new AppAction(DssViewer.EXCEEDENCE_STRING, _mp,
			"Display exceedence plot of selected pathnames") {
		public void actionPerformed(ActionEvent e) {
...
//CB				&& getFP().isMonthlyTimeStep();
			return super.canAct()  && getMessagePanel().isCompActive()
				&& !_mp.isSelectedFilesThreadAlive() && !_mp.isUnselectedFilesThreadAlive()			
				&& getFilterPanel().isFilterActive() && (getFilterPanel().rowsSelected()
					|| getFilterPanel().areDashboardRowsSelected())
	 ...
	 * CB added
	 */
	AppAction MonthlyAverageAction = new AppAction(DssViewer.MONTHLY_AVERAGE_STRING, _mp,
...
//CB				&& getFP().isMonthlyTimeStep();
	...
	AppAction AnnualTotAction = new AppAction(DssViewer.ANNUAL_TOTAL_STRING, _mp,
			"Display annual total plot of selected pathnames") {
		public void actionPerformed(ActionEvent e) {
//CB			getFP().retrieve(DssViewer.PLOT, false, true);
		...
		public boolean canAct() {
//CB			return super.canAct() && getFP().isFilterValid() && getFP().rowsSelected()
//CB					&& getFP().isMonthlyTimeStep();
			return super.canAct()  && getMessagePanel().isCompActive()
				&& !_mp.isSelectedFilesThreadAlive() && !_mp.isUnselectedFilesThreadAlive()
				&& getFilterPanel().isFilterActive() && (getFilterPanel().rowsSelected()
	...
	AppAction AnnualTotExceedAction = new AppAction(DssViewer.ANNUAL_TOTAL_EXCEEDENCE_STRING, _mp,
...
//CB					&& getFP().isMonthlyTimeStep();
	...
	// Project
	AppAction ViewerNewFileAction = new AppAction("New", _mp, "Reset DSS file settings") {
		public void actionPerformed(ActionEvent e) {
	...
	AppAction ViewerOpenFileAction = new AppAction("Open", _mp, "Open a project file") {
		public void actionPerformed(ActionEvent e) {
			openViewerFile();
		}
	};

	AppAction ViewerSaveFileAction = new AppAction("Save As", _mp, "Save a viewer project file") {
		public void actionPerformed(ActionEvent e) {
			saveViewerFile();
		}
	};
	...
	AppAction BaseDVOpenFileAction = new AppAction("Dvar File", _mp, "Open the Base DV file") {
		public void actionPerformed(ActionEvent e) {
...
//CB			openDSSFile("Comp3DV");
	...
	AppAction BaseSVOpenFileAction = new AppAction("Svar File", _mp, "Open the Base SV file") {
		public void actionPerformed(ActionEvent e) {
...
//CB			openDSSFile("Comp3SV");
	...
	AppAction BaseDVOpenAction = new AppAction("DV:", _mp, "Open the Base DV file") {
		public void actionPerformed(ActionEvent e) {
//CB			openDSSFile("BaseDV");
	...
	AppAction Comp1DVOpenAction = new AppAction("DV:", _mp, "Open the 1st Comparison DV file") {
		public void actionPerformed(ActionEvent e) {
//CB			openDSSFile("Comp1DV");
	...
	AppAction Comp2DVOpenAction = new AppAction("DV:", _mp, "Open the 2nd Comparison DV file") {
		public void actionPerformed(ActionEvent e) {
//CB			openDSSFile("Comp2DV");
	...
	AppAction Comp3DVOpenAction = new AppAction("DV:", _mp, "Open the 3rd Comparison DV file") {
		public void actionPerformed(ActionEvent e) {
//CB			openDSSFile("Comp3DV");
	...
	AppAction BaseSVOpenAction = new AppAction("SV:", _mp, "Open the Base SV file") {
		public void actionPerformed(ActionEvent e) {
//CB			openDSSFile("BaseSV");
	...
	AppAction Comp1SVOpenAction = new AppAction("SV:", _mp, "Open the 1st Comparison SV file") {
		public void actionPerformed(ActionEvent e) {
//CB			openDSSFile("Comp1SV");
	...
	AppAction Comp2SVOpenAction = new AppAction("SV:", _mp, "Open the 2nd Comparison SV file") {
		public void actionPerformed(ActionEvent e) {
//CB			openDSSFile("Comp2SV");
	...
	AppAction Comp3SVOpenAction = new AppAction("SV:", _mp, "Open the 3rd Comparison SV file") {
		public void actionPerformed(ActionEvent e) {
//CB			openDSSFile("Comp3SV");
	...
	AppAction CompModeAction = new AppAction("Comp", _mp, "Compare Mode") {
		...
		/* CB
		public boolean canAct() {
			if (!getMP().isCompDiffActive() && getMP().isBaseActive()) {
				// getMP().setMode("Base");
				return true;
			} else if (getMP().isBaseActive())
				return true;
			return false;
		} */
	...
	//CB added
	AppAction WateryearModeAction = new AppAction("Oct - Sep", _mp, "Wateryear Annual Mode") {
		public void actionPerformed(ActionEvent e) {
	...
	//CB added
	AppAction CalendarYearModeAction = new AppAction("Jan - Dec", _mp, "Calendar Year Annual Mode") {
		public void actionPerformed(ActionEvent e) {
	...
	//CB added
	AppAction FederalContractYearModeAction= new AppAction("Mar - Feb", _mp, "Federal Contract Year Annual Mode") {
		public void actionPerformed(ActionEvent e) {
	...
	/*
	 * AppAction CompModeAction = new AppAction("Comp", _mp, "Compare Mode") { public void
	 * actionPerformed(ActionEvent e) { getMP().setMode("Comp"); } public boolean canAct() { return
	 * getMP().isCompDiffActive(); } };
	 */
...
/*	public void loadDefaultFiles() {
		...
		// filter for dss files
		FilenameFilter filter = new FilenameFilter() {
			public boolean accept(File dir, String name) {
				return name.toLowerCase().endsWith(".dss");
			}
		};

		// Base DSS files
		String defaultLocation = "c:/ztp/schematic_dss/defaults/base/";
		setDefaultFile(filter, "Base", defaultLocation, "Loading Base DSS files ...");

		// Comp DSS files
		defaultLocation = "c:/ztp/schematic_dss/defaults/comp/";
		setDefaultFile(filter, "Comp", defaultLocation, "Loading Comp DSS files ...");
	} */

/*	public void setDefaultFile(FilenameFilter filter, String dssType, String loc, String message) {
		_mp.setStatus(message);
		...
		if (children == null) {
			...
			System.out.println("No dss files");
		} else {
...
//				System.out.println("file: " + filename);
				...
				if (dssType.startsWith("Base"))
					cnt = "";
				else
					cnt = String.valueOf(i + 1);

				if (filename.toLowerCase().contains("sv"))
					dss = dssType + cnt + "SV";
				else
					dss = dssType + cnt + "DV";

				String fn = loc + filename;
				setDSSFile(fn, dss);
			}
		}
		_mp.setStatus();
	...
	public String openViewerFile(String filename) {
		...
		if (!new File(filename).exists()) {
			JOptionPane.showMessageDialog(_mp, "DSS project file, " + filename + " does not exist",
...
//CB		getFP().newViewerFile();  //CB commented out because it does redundant things
...
//CB			getFilterPanel().refreshFilter("DV");
			getFilterPanel().refreshFilter(FilterPanel.DVAR); //TODO finish refreshFilter and may sure this call works?
		...
		_mp.doMonthlyDataWork();
		_mp.updateDSSMenu(filename, true);  // TODO test this call with boolean arg added
		return filename;
	}
	...
	void saveViewerFile() {
		...
		_mp.getMessagePanel().setProjectName(fn); //CB added to make sure it is there
		_mp.updateDSSMenu(fn, true);  // TODO test this call with boolean arg added
		_mp.setStatus();
...
//CB		setDSSFile(fn, dss);
		...
		//CB added block.  TODO: if the filter button is changed to act on more than just the Base file, need to to something here
		if (_mp.isDateBoxListEmpty())
			_mp.setDateBoxModel(new DefaultComboBoxModel(_mp.loadDateBoxList()));
/*		_mp.loadAllVariableData(_mp.getAllVariables(), dssType);
		String date = _mp.getSchematicDate();
		if (date != null && !date.trim().equals("")) {
			if (date.indexOf("-") == -1) //CB long-term averages need time for recalculation (see below)
				_mp.updateSchematicValues();
		}
		calculateLongTermAverages(); */
	...
	public String getFileName(FFilter filter, int dialogType, String fileType) {
		...
		// if (DEBUG) System.out.println("FFilter Description:"+filter.getDescription());
