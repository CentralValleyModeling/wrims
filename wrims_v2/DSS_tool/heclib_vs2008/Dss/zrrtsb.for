...
C
 ...
 20   FORMAT (T8,'----DSS---Debug:  Enter ZRRTSB;  Unit:',I5)
C
C
C
...
C              Read doubles
               ...
               N = NREAD * IWORD(10)
               CALL ZGTREC (IFLTAB, DVALUES(NSTART), N, IADD, .FALSE.)
               IF (IFLTAB(KDSWAP).NE.0) CALL ZDSWAP(DVALUES(NSTART), N)
            ELSE
               IADD = INFO(NPPWRD+KIADAT) + NPOS - 1
               CALL ZGTREC(IFLTAB, SVALUES(NSTART), NREAD,IADD,.FALSE.)
            ENDIF
         ELSE
...
C              Translate single (file) into double (requested)
               ...
               IADD = INFO(NPPWRD+KIADAT) + NPOS - 1
               CALL ZGTREC (IFLTAB, BUFF, NREAD, IADD, .FALSE.)
               M = 0
               DO 80 I=NSTART, ILIM
                  M = M + 1
                  DVALUES(I) = BUFF(M)
...
C              Translate doubles (file) into singles (requested)
               ...
               IADD = INFO(NPPWRD+KIADAT) + (NPOS*2) - 2
               N = NREAD * IWORD(10)
               CALL ZGTREC (IFLTAB, DBUFF, N, IADD, .FALSE.)
...
C              flags live after the data block for doubles
     ...
     *                (INFO(NPPWRD+KILNDA) * 2)
               CALL ZGTREC (IFLTAB, JQUAL(NSTART), NREAD, IADD, .FALSE.)
            ELSE
