      SUBROUTINE ZSET ( CFLG, CSTR, NUMB)
C
...
C
 ...
 20   FORMAT (T2,'-----DSS---Debug:  Enter ZSET;  Flag: ',A,
     * ',  Numb:',I9,',  String: ',A)
C
C     Set Program Name
...
C     Set this file in an exclusive assign mode (Harris only)
      ...
      LEXCL = .TRUE.
C
C     Force this file to a multi-user mode (do not go into an advisory mode!)
...
C     Set the time for the next record to be written
      ...
      CTIME = CSTR
C
C     Set the time zone offset and name for the next record to be written
...
C     Should we show the handle instead of the unit number
      ...
      ENDIF
C
C     Write (and read) to memory, instead of disk?
C     The size of the file is passed in as the number
C     The file name for zopen must be a single digit number (0-9)
      ELSE IF (CFLAG.EQ.'MEMO') THEN
      IF (CSTRIN.EQ.'ON') THEN
         LMEMORY = .TRUE.
         MEMSIZE = NUMB
      ELSE
...
C     TEMP REMOVAL OF ERROR WRITE FOR USE WITH ZSET4.  BILL CHARLEY
      IF (MLEVEL.GE.2) WRITE (MUNIT,910) CFLAG, CSTR, NUMB
