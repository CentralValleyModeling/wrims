     ...
     * NFOUND)
C
C     Given an array of tags (may be one tag), ZTAGPA finds
...
C     unit JUNIT (in a form readable by ZRDCAT) instead of being
...
C
      ...
      PARAMETER (MXTAGS=50)
      INTEGER NPATH(*), IHCODE(MXTAGS)
C
C
C
      IF (MLEVEL.GE.11) WRITE (MUNIT,20) IFLTAB(KUNIT), NTAGS, CTAGS(1)
 20   FORMAT (T6,'-----DSS---Debug:  Enter ZTAGPA;  Unit:',I5,/,
     * T11,'NTAGS:',I4,'   First Tag: -',A,'-')
C
C
C
C     Check that IFLTAB is valid (e.g., the DSS file is open)
      IF (IFLTAB(1).NE.6) CALL ZERROR (IFLTAB, 5, 'ZTAGPA',
     * 0, IFLTAB, ' ', 0, ' ',0)
C
C
C
...
C
      CALL ZGTREC (IFLTAB, ILBUFF, NTAGBK, IFLTAB(KTAGBK), .FALSE.)
      IBPOS = 3
C
 100  CONTINUE
...
C     No more bins or pathnames left.  All done
 ...
 820  FORMAT (T6,'-----DSS---Debug:  Exit  ZTAGPA;  Numb Found:',I4)
      RETURN
C
C
 900  CONTINUE
      CALL ZABORT (IFLTAB, 100, 'ZTAGPA', 0, IFLTAB(KTABLE), ' ')
      RETURN
C
 910  CONTINUE
...
C
 ...
 921  FORMAT (/' -----DSS--- ZTAGPA:  Error during Write, Unit:',I5,
     * /,' Error:',I5,/)
      GO TO 800
C
      END
