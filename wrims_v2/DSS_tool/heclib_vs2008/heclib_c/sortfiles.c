...
tempname()
{
  ...
  char *name = xmalloc(len + 22);
  struct tempnode *node =
    (struct tempnode *) xmalloc(sizeof (struct tempnode));

#ifdef MSDOS
  if (len && prefix[len - 1] != '\\')
    sprintf(name, "%s\\srt%5.5x.%3.3x", prefix, getpid(), ++seq);
  else
    sprintf(name, "%ssrt%5.5x.%3.3x", prefix, getpid(), ++seq);
#else /* not MSDOS */
  if (len && prefix[len - 1] != '/')
...
/* void
sortfilesinterface (char *file1, int len1, char *file2, int len2, int *status) */
void
sortfilesinterface_ (char *file1, char *file2, int *status, int len1, int len2)
{
