...
//		JMenuItem menuItem = new JMenuItem(plotAction);
		...
		JMenuItem menuItem = new JMenuItem(plotAction);
		if (!plotAction.canAct()) menuItem.setEnabled(false);
...
//		submenu.add(menuItem);
		...
		if (!monthlyAction.canAct()) menuItem.setEnabled(false);
		menuItem.setMnemonic(KeyEvent.VK_M);
		submenu.add(menuItem);
		menuItem = new JMenuItem(tableAction);
		if (!tableAction.canAct()) menuItem.setEnabled(false);
		menuItem.setMnemonic(KeyEvent.VK_T);
		submenu.add(menuItem);
		menuItem = new JMenuItem(allAction);
		if (!allAction.canAct()) menuItem.setEnabled(false);
	...
	//*********************************************************************************************
	//CB TO DO: create a DssFrameRelatedPopupAction class so that the below objects don't have so
...
//		Icon icon = iconImage("images/Copy24.gif");
		plotAction = new DssFrameRelatedAction(DssViewer.PLOT_STRING, SchematicUtils.schematic) {
						...
						if (e.getSource() instanceof JMenuItem) {
							...
							if (((JMenuItem)e.getSource()).getParent() instanceof JPopupMenu) {
								...
								if (invoker instanceof JMenu) {
									if (((JMenu)invoker).getText().toLowerCase().indexOf("plot") == -1) {//CB had to add this because i put 3 actions in one plot type submenu
										variableType = ((JMenu)invoker).getText(); //CB now it is "Plot Type"
									} else {  //CB had to add this because i put 3 actions in one plot type submenu
										if (((JMenu)invoker).getParent() instanceof JPopupMenu) {
											invoker = ((JPopupMenu)((JMenu)invoker).getParent()).getInvoker();
											if (invoker instanceof JMenu) {
	...
	{
//		Icon icon = iconImage("images/Copy24.gif");
		exceedenceAction = new DssFrameRelatedAction(DssViewer.EXCEEDENCE_STRING, SchematicUtils.schematic) {
			public void actionPerformed(ActionEvent e) {
				if (canAct()) {
					String outputType = "";
					if (((Schematic)getApp())._DssFrame.getFP().filterByBpart
						...
						if (e.getSource() instanceof JMenuItem) {
							...
							if (((JMenuItem)e.getSource()).getParent() instanceof JPopupMenu) {
								...
								if (invoker instanceof JMenu) {
									if (((JMenu)invoker).getText().toLowerCase().indexOf("plot") == -1) {//CB had to add this because i put 3 actions in one plot type submenu
										variableType = ((JMenu)invoker).getText(); //CB now is "Plot Type"
									} else {  //CB had to add this because i put 3 actions in one plot type submenu
										if (((JMenu)invoker).getParent() instanceof JPopupMenu) {
											invoker = ((JPopupMenu)((JMenu)invoker).getParent()).getInvoker();
											if (invoker instanceof JMenu) {
	...
	{
//		Icon icon = iconImage("images/Copy24.gif");
		annualTotAction = new DssFrameRelatedAction(DssViewer.ANNUAL_TOTAL_STRING, SchematicUtils.schematic) {
			public void actionPerformed(ActionEvent e) {
				if (canAct()) {
					String outputType = "";
					if (((Schematic)getApp())._DssFrame.getFP().filterByBpart
	...
	DssFrameRelatedAction annualTotExceedAction = null;  //extends AbstractAction
	{
//		Icon icon = iconImage("images/Copy24.gif");
		annualTotExceedAction = new DssFrameRelatedAction(DssViewer.ANNUAL_TOTAL_EXCEEDENCE_STRING, SchematicUtils.schematic) {
			public void actionPerformed(ActionEvent e) {
				if (canAct()) {
					String outputType = "";
					if (((Schematic)getApp())._DssFrame.getFP().filterByBpart
	...
	{
//		Icon icon = iconImage("images/Copy24.gif");
		monthlyAverageAction = new DssFrameRelatedAction(DssViewer.MONTHLY_AVERAGE_STRING, SchematicUtils.schematic) {
			public void actionPerformed(ActionEvent e) {
				if (canAct()) {
					String outputType = "";
					if (((Schematic)getApp())._DssFrame.getFP().filterByBpart
...
//		Icon icon = iconImage("images/Copy24.gif");
						...
						if (e.getSource() instanceof JMenuItem) {
							...
							if (((JMenuItem)e.getSource()).getParent() instanceof JPopupMenu) {
								Component invoker = ((JPopupMenu)((JMenuItem)e.getSource())
									.getParent()).getInvoker();
								if (invoker instanceof JMenu) {
									variableType = ((JMenu)invoker).getText();  //e.e., like "Storage (EOP)" or "Inflows"
								}
							}
						}
						actionPerfomedHelper(outputType, variableType);
	...
	{
//		Icon icon = iconImage("images/Copy24.gif");
		tableAction = new DssFrameRelatedAction(DssViewer.TABLE_STRING, SchematicUtils.schematic) {
			public void actionPerformed(ActionEvent e) {
