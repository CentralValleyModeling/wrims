      SUBROUTINE ZCUT(IFLTAB, CPATH, LDELETE, IBUFFER, MAXBUFF, ISTATUS)
C
C     Copy a single record into a buffer area.  (copy)
C     If LDELETE is true, delete the record afterwards (cut).
C     Can use ZPASTE to paste the record to another dss file
C
...
C
 ...
 20      FORMAT (T6,'-----DSS---Debug: Enter ZCUT',/,
     *   T5,'Unit:',I5,'  Path: ',A)
      ENDIF
C
      CALL ZCUTSZ (IFLTAB, CPATH, ISIZE, ISTATUS)
...
C     Read in the main information block
      CALL ZRDINF(IFLTAB, CPATH, JHEAD, JDATA, ISTATUS)
      IF (ISTATUS.NE.0) GO TO 900
C
...
C     Read the internal header
      IF (NIHEAD.GT.0) THEN
         ...
         CALL ZGTREC (IFLTAB, IBUFFER(ILOC), NIHEAD, NADD, .FALSE.)
         ILOC = ILOC + NIHEAD
      ENDIF
C
C     Read the Compression header
      IF (NCHEAD.GT.0) THEN
         ...
         CALL ZGTREC (IFLTAB, IBUFFER(ILOC), NCHEAD, NADD, .FALSE.)
         ILOC = ILOC + NCHEAD
      ENDIF
C
C     Read the User header
      IF (NUHEAD.GT.0) THEN
         ...
         CALL ZGTREC (IFLTAB, IBUFFER(ILOC), NUHEAD, NADD, .FALSE.)
         ILOC = ILOC + NUHEAD
      ENDIF
C
C     Read the Data block
      IF (NDATA.GT.0) THEN
         ...
         CALL ZGTREC (IFLTAB, IBUFFER(ILOC), NDATA, NADD, .FALSE.)
         ILOC = ILOC + NDATA
      ENDIF
C
C
...
C
 ...
 820  FORMAT (T6,'-----DSS---Debug: Exit ZCUT, Status:',I4)
*      CALL FLUSH(MUNIT)                                         Mu
      RETURN
C
 900  CONTINUE
 ...
 901  FORMAT (/,' -----DSS---ZCUT:  ERROR;  Record does not exist: ',A)
      GO TO 800
C
 920  CONTINUE
      ISTAT = -2
 ...
 921  FORMAT (/,' -----DSS---ZCOREC:  ERROR;  Insufficient buffer',
