...
C
 ...
 20   FORMAT (/,T10,'-----DSS---Debug: Enter ZNWRIT,  Unit:',I5,/,
...
C
C
C     Update the current date and time, if settings request it
      IF (LUPRTIME) THEN
...
C     Write a new pathname bin
      IF(MLEVEL.GE.14)WRITE(MUNIT,*)'-ZNWRIT:  Write new path bin'
      CALL ZNWBIN (IFLTAB)
C
C     Write address of new pathname bin to hash location in table
      IF(MLEVEL.GE.14)WRITE(MUNIT,*)'-ZNWRIT:  Save bin add in index'
      NADD = NPERM + IHASH
...
C     No more space in this bin - create a new bin
      IF(MLEVEL.GE.14)WRITE(MUNIT,*)'-ZNWRIT:  Add another path bin'
      IPNBIN(JPNBIN) = -1
      CALL ZNWBIN (IFLTAB)
      ENDIF
      ENDIF
...
C     Now store this bin with the new information
      IF(MLEVEL.GE.14)WRITE(MUNIT,*)'-ZNWRIT:  Store path bin'
      I = INT(IFLTAB(KBNSIZ))
...
C     Store the information block
      ISIZE = NPPWRD + NINFO
      IF(MLEVEL.GE.14)WRITE(MUNIT,*)'-ZNWRIT:  Store info block'
...
C     Reset flags
 ...
 820  FORMAT (T8,'-----DSS---Debug: EXIT ZNWRIT')
      RETURN
C
      END
