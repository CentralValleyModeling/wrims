 ...
 * @auther Clay Booher
 * CB - this class should be rewritten for clarity - debugging it is a pain.
 *
 */
public class MessagePanel {
	...
	/** CB added */
	public static final String[] _twSelections = { "OCT1921 - SEP2003", "OCT1921 - SEP1994",
		"MAY1928 - OCT1934", "JUN1986 - SEP1992", "OCT1975 - SEP1977", "OCT1983 - SEP1993", };
	private int _annualType;   //CB added

	private Preferences _userPrefs;  //CB added
	public static String CHECKED = "checked";

	public static String[] labelText = { " Project Name: ", "  Mode: ",  //CB this is a mess
			" TW:  ", " Base Files ", " DV:    ", "               SV: ",
			" Comp Files 1", " DV:    ", "               SV: ",
	...
	// CB TODO decide if want to set checkbox prefs here or in a method called writeDssFileCheckedPreferences
	ItemListener al1 = new ItemListener() { //CB TODO, alter instance variables to greatly reduce code here and elsewhere in this class
		public void itemStateChanged(ItemEvent e) {
			AppAction.updateAllActions();
			if (e.getSource() == _basebox) { //CB added elses to prevent 3 (or more later) "if" checks.  Later made the base box NOT changeable
//CB always checked now				_userPrefs.putBoolean(getProjectName() + CHECKED + "0", _basebox.isSelected()); //CB TO DO:  use array of checkboxes and index for String creation
				_mainPanel.updateSchematicValues();  //_basebox is always selected
			} else if (e.getSource() == _comp1box) {
				_userPrefs.putBoolean(getProjectName() + CHECKED + "1", _comp1box.isSelected()); //CB TO DO:  use array of checkboxes and index for String creation
				_mainPanel.updateSchematicValues();    // if selected or unselected, need to update alt. 2 values (TODO - if files both blank, maybe not)
			} else if (e.getSource() == _comp2box) {
				_userPrefs.putBoolean(getProjectName() + CHECKED + "2", _comp2box.isSelected()); //CB TO DO:  use array of checkboxes and index for String creation
				if (!_comp1box.isSelected()) // if selected or unselected, need to update alt. 2 values IF _comp1box is NOT selected (TODO - if files both blank, maybe not)
					_mainPanel.updateSchematicValues();
			} else if (e.getSource() == _comp3box) {
				_userPrefs.putBoolean(getProjectName() + CHECKED + "3", _comp3box.isSelected()); //CB TO DO:  use array of checkboxes and index for String creation
				if (!_comp1box.isSelected() && !_comp2box.isSelected()) // if selected or unselected, need to update alt. 2 values IF _comp1box AND _comp2box are NOT selected (TODO - if files both blank, maybe not)
					_mainPanel.updateSchematicValues();
			}
		}
	};
	...
	public MessagePanel(DssAppAction aa, MainPanel mainPanel, int schematicUnits) {  //CB TODO call the other constrauctor
		_aa = aa;
	...
	JPanel createMessagePanel(int schematicUnits) {
		...
		_userPrefs = Preferences.userNodeForPackage(getClass());  // CB added for remembering user checks upon last close
		...
		twbox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (!_mainPanel.dateBoxContainsTimeWindow(tw)) {
					_mainPanel.updateSchematicDateBox((String)tb.getModel().getSelectedItem());
				}
//				System.out.print("");
			}
		});
	...
	void setProjectName(String nm) {
		...
		readDssFileCheckedPreferences();  //CB added TODO - verify it is needed here remove it if not)
	}
	...
	/**
	 * CB added - return a copy of the "built-in time windows of historical importance.
	 * @return
	 */
...
//	public HashMap<String, Object> getDSSFileNames(String type) {
	public HashMap<Integer, Object> getDSSFileNames(String type) {  //CB TODO redo instance variables so much less code here and everywhere is necessary!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//CB		HashMap<String, Object> hm = new HashMap<String, Object>(20);  //CB 5/2008 changed to size 20 (from default of 10) as it had bug noted below.
		HashMap<Integer, Object> hm = new HashMap<Integer, Object>(20);  //CB 5/2008 changed to size 20 (from default of 10) as it had bug noted below.
...
//CB				hm.put("2", _Comp2DVFileName);  //CB Java 1.6 or Hashmap bug when hm size = 10.  This line was overwriting the HashMap entry taken by hm.put("Base", _BaseDVFileName); line above!!!!!!!!!!!!!
...
//CB				if (hm.get("Base") != null) { //CB added ability for > 1 associated file
				if (hm.get(FilterPanel.BASE_DV_FILE_INT/4) != null) { //CB added ability for > 1 associated file
...
//CB				if (hm.get("1") != null) { //CB added ability for > 1 associated file
				if (hm.get(FilterPanel.COMP1_DV_FILE_INT/4) != null) { //CB added ability for > 1 associated file
					Vector<String> files = new Vector<String>(2);
...
//				if (hm.get("2") != null) { //CB added ability for > 1 associated file
				if (hm.get(FilterPanel.COMP2_DV_FILE_INT/4) != null) { //CB added ability for > 1 associated file
					Vector<String> files = new Vector<String>(2);
...
//CB				if (hm.get("3") != null) { //CB added ability for > 1 associated file
				if (hm.get(FilterPanel.COMP3_DV_FILE_INT/4) != null) { //CB added ability for > 1 associated file
					Vector<String> files = new Vector<String>(2);
...
//CB					hm.put("3", _Comp3SVFileName);
	...
	/**
	 * CB added to allow loading of the "rest" of the data for the monthyl value boxes in a separate thread.
	 * @param type
	 * @return
	 */
	public HashMap<Integer, Object> getUnselectedDSSFileNames(String type) {  //CB TODO redo instance variables so much less code here and everywhere is necessary!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		HashMap<Integer, Object> hm = new HashMap<Integer, Object>(20);  //CB 5/2008 changed to size 20 (from default of 10) as it had bug noted below.
		if (type.equals("DVAR") || type.equals("BOTH")) { //CB added BOTH
			if (!_basebox.isSelected() && !_BaseDVFileName.trim().equals(""))  //CB
				hm.put(FilterPanel.BASE_DV_FILE_INT/4, _BaseDVFileName); //CB
		...
		if (type.equals("SVAR") || type.equals("BOTH")) { //CB added BOTH
			...
			if (!_comp3box.isSelected() && !_Comp3SVFileName.trim().equals("")) {  //CB)
				if (hm.get(FilterPanel.COMP3_SV_FILE_INT/4) != null) { //CB added ability for > 1 associated file
					Vector<String> files = new Vector<String>(2);
					files.add(_Comp3DVFileName);
					files.add(_Comp3SVFileName);
...
/* CB	void setDSSFile(String fn, String dss) {
		if (DEBUG)
			System.out.println("setDSSFile() " + dss + " " + fn);
		if (dss.equals("BaseDV")) {
			_BaseDVFileName = fn;
		} else if (dss.equals("Comp1DV"))
			_Comp1DVFileName = fn;
		else if (dss.equals("Comp2DV"))
			_Comp2DVFileName = fn;
		else if (dss.equals("Comp3DV"))
			_Comp3DVFileName = fn;
		else if (dss.equals("BaseSV")) {
			_BaseSVFileName = fn;
		} else if (dss.equals("Comp1SV"))
			_Comp1SVFileName = fn;
		else if (dss.equals("Comp2SV"))
			_Comp2SVFileName = fn;
		else if (dss.equals("Comp3SV"))
			_Comp3SVFileName = fn;
	...
	private void readDssFileCheckedPreferences() {
		_comp1box.setSelected(_userPrefs.getBoolean(getProjectName() + CHECKED + "1", true));
		_comp1box.addItemListener(al1);
		_comp2box.setSelected(_userPrefs.getBoolean(getProjectName() + CHECKED + "2", true));
		_comp2box.addItemListener(al1);
		_comp3box.setSelected(_userPrefs.getBoolean(getProjectName() + CHECKED + "3", true));
	}
...
//		if (isBaseActive()) selectedKeys.add("Base");
		if (isBaseActive()) selectedKeys.add(FilterPanel.BASE_DV_FILE_INT/4);
...
//		String[] keys = new String[selectedKeys.size()];
	...
	private boolean areArgumentNumberOrMoreDVFilesActive(int number) {
		if (number <= 0) //In case of a meaningless argument
			return true;
		int numActive = 0;
		if (_basebox.isSelected() && !_BaseDVFileName.trim().equals(""))  //it is empty!!!!!!!!!!!!!!!!!!!!!!!!!
			++numActive;
		if (numActive == number)
			return true;
		if (_comp1box.isSelected() && !_Comp1DVFileName.trim().equals(""))
	...
	public boolean isDiffActive() {
		...
		if (!_BaseDVFileName.equals("") && _basebox.isSelected()
				&& !_Comp1DVFileName.equals(""))
			return true;
		else if (!_BaseSVFileName.equals("") && _basebox.isSelected()
				&& !_Comp1SVFileName.equals(""))
			return true;
		setModeMessage("Comp");
		return false;
	} */
	...
	public boolean isDiffActive() {
		...
		if (!getMode().equals("Comp"))
			setMode("Comp");  //CB TODO either change these text modes to int (or a single boolean because there are two) have the mode detected at output time
		return false;
	}
	...
	public boolean isDvarCompActive() {
		if (!_BaseDVFileName.equals("") && _basebox.isSelected())
			return true;
		if (!_Comp1DVFileName.equals("") && _comp1box.isSelected())
			return true;
		if (!_Comp2DVFileName.equals("") && _comp2box.isSelected())
			return true;
		if (!_Comp3DVFileName.equals("") && _comp3box.isSelected())
			return true;
		return false;
	} */
	...
	public boolean isSvarCompActive() {
		if (!_BaseSVFileName.equals("") && _basebox.isSelected())
			return true;
		if (!_Comp1SVFileName.equals("") && _comp1box.isSelected())
			return true;
		if (!_Comp2SVFileName.equals("") && _comp2box.isSelected())
			return true;
		if (!_Comp3SVFileName.equals("") && _comp3box.isSelected())
			return true;
		return false;
	} */
