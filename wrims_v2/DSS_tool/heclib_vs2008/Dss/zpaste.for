      SUBROUTINE ZPASTE (IFLTAB, IBUFFER, ISTATUS)
C
C     Paste a single record (from zcut) into a dss file.
C
...
C
      IF (MLEVEL.GE.11) THEN
         WRITE (MUNIT, 20)
 20      FORMAT (T6,'-----DSS---Debug: Enter ZPASTE')
      ENDIF
C
...
C     If the pathname was not found by ZCHECK write new pointers
      IF (.NOT.LFOUND) THEN
         CALL ZNWRIT (IFLTAB, CPATH, NPATH, NIHEAD, NCHEAD, NUHEAD,
     *                NDATA)
      ELSE
         IF (LPROTC) GO TO 930
         CALL ZOWRIT (IFLTAB, CPATH, NPATH, NIHEAD, NCHEAD, NUHEAD,
     *                NDATA)
      ENDIF
C
...
C     Now store the headers and data set
      ...
      IF (NIHEAD.GT.0) THEN
         ...
         CALL ZPTREC (IFLTAB, IBUFFER(ILOC), NIHEAD, NADD, .FALSE.)
         ILOC = ILOC + NIHEAD
      ENDIF
C
      IF (NCHEAD.GT.0) THEN
         NADD = INFO(NPPWRD+KIACHE)
         CALL ZPTREC (IFLTAB, IBUFFER(ILOC), NCHEAD, NADD, .FALSE.)
         ILOC = ILOC + NCHEAD
      ENDIF
C
      IF (NUHEAD.GT.0) THEN
         NADD = INFO(NPPWRD+KIAUHE)
         CALL ZPTREC (IFLTAB, IBUFFER(ILOC), NUHEAD, NADD, .FALSE.)
         ILOC = ILOC + NUHEAD
      ENDIF
C
      IF (NDATA.GT.0) THEN
         NADD = INFO(NPPWRD+KIADAT)
         CALL ZPTREC (IFLTAB, IBUFFER(ILOC), NDATA, NADD, .FALSE.)
         ILOC = ILOC + NDATA
      ENDIF
C
      IF (MLEVEL.GE.3) THEN
 ...
 200        FORMAT(' --ZWRITE: ',A)
     ...
     *                       CPATH(1:NPATH)
 220        FORMAT(' -----DSS---ZWRITE Unit',I5,'; Vers.',I5,':',2X,A)
         ENDIF
      ENDIF
C
C
...
C
 ...
 820  FORMAT (T6,'-----DSS---Debug: Exit ZPASTE, Status:',I4)
*      CALL FLUSH(MUNIT)                                    Mu
      RETURN
C
C
 ...
 901  FORMAT (/' -----DSS---ZPASTE: ERROR; Invalid paste buffer block')
      GO TO 800
C
 910  CONTINUE
      ISTAT = -5
      WRITE (MUNIT, 911)
 911  FORMAT (/' -----DSS---ZPASTE:  ERROR;  Files are of different',
...
C
 ...
 921  FORMAT (' -----DSS---ZPASTE:  ERROR;  File has Read Access Only',
     * /,' Pathname: ',A)
      GO TO 800
C
 930  CONTINUE
 ...
 931  FORMAT (' -----DSS---ZPASTE:  Write Protection for Existing',
...
C
 ...
 941  FORMAT (/,' *****DSS*** ZPASTE:  ERROR  - UNABLE TO ',
     * ' COPY DATA',/,' Status: ',I8)
      GO TO 800
C
C
