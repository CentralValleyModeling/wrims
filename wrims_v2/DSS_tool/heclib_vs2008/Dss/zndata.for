      SUBROUTINE ZNDATA (IFLTAB, IFPOS, JULS, ISECS, CPATH, NPATH, 
...
C
 ...
 20   FORMAT (T6,'-----DSS---Debug:  Enter ZNDATA;  Unit:',I5)
C
C
...
C         If it is way off, then it is a file error
      ...
      ENDIF
      CALL ZGTREC (IFLTAB, IPBIN, IFLTAB(KBNSIZ), IADD, .FALSE.)
C
C
...
C
 ...
 920  FORMAT(' -----DSS---Error:  File error detected in ZNDATA')
C      CALL ZERROR (IFLTAB, 100, 'ZNDATA', 0, IFLTAB(KTABLE), ' ', 0,
C     * ' ', 0)
      IF (ISTAT.EQ.-2) THEN
      IF (MLEVEL.GE.1) WRITE(MUNIT,930) IADD
 930  FORMAT(' ZNDATA Error: File address out of range:',I10)
      ELSE IF (ISTAT.EQ.-3) THEN
	IF (MLEVEL.GE.1) WRITE(MUNIT,930) IADD
      ELSE IF (ISTAT.EQ.-4) THEN
      IF (MLEVEL.GE.1) WRITE(MUNIT,940) IPBIN(JPBIN)
 940  FORMAT(' ZNDATA Error: Pathname status out of range:',I10)
      ENDIF
