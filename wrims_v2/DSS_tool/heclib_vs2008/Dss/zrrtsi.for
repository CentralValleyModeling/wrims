...
C     If a debug level is on, print out information
 ...
 20   FORMAT (T10,'----- Entering ZRRTSX for unit',I5,' -----')
      ENDIF
C
...
C              Time pattern data is always single precision.
               NVALS = KLBUFF
               CALL ZRRTPA (IFLTAB, CPATH, BUFF, NVALS, CUNITS,
 ...
 50            CONTINUE
            ELSE
               NVALS = KVALS
               CALL ZRRTPA (IFLTAB, CPATH, SVALUES, NVALS, CUNITS,
     *                      CTYPE, IUHEAD, KHEADU, NUHEAD, ISTAT)
            ENDIF
            LCOORDS = .FALSE.
            LQREAD = .FALSE.
...
C
 ...
 60   FORMAT (T10,'No time Window set.  Interval:',I7,'  Maximum',
     * ' number of data values:',I7,/,T10,'Computed Maximum Times:')
...
C     Read the internal header (containing offset, units, etc.)
      ...
      IF (NIHEAD.GT.0)
     * CALL ZGTREC (IFLTAB, IIHEAD, NIHEAD, INFO(NPPWRD+KIAIHE), .TRUE.)
	...
	LCOORDS = .FALSE.
      ...
      ELSE IF (NIHEAD.GE.12) THEN
	   ...
	   IF (NIHEAD.GE.24) THEN
	      CALL ZMOVWD(IIHEAD(13), COORDS(1), 6)         
            DO 250 I=1,6
...
C
 800  CONTINUE
     ...
     * CUNITS, CTYPE
 820  FORMAT(T8,'----- Exiting ZRRTS, Number of data values:',I7,
...
C
 ...
 901  FORMAT (/,' *****DSS*** ZRRTS:  ERROR  - ILLEGAL PATHNAME',
...
C
 910  CONTINUE
     ...
     * INTL, CPATH
 911  FORMAT (/,' *****DSS*** ZRRTS:  ERROR  - NON-STANDARD TIME',
...
C
 ...
 921  FORMAT (/,' *****DSS*** ZRRTS:  ERROR  - NUMBER OF VALUES',
...
C
 ...
 931  FORMAT (/,' *****DSS*** ZRRTS:  ERROR  - ILLEGAL STARTING',
...
C
 ...
 941  FORMAT (/,' *****DSS*** ZRRTS:  ERROR  - ILLEGAL STARTING',
...
C
 950  CONTINUE
     ...
     * CPATH(1:NPATH)
 951  FORMAT(/,' *****DSS*** ZRRTS:  ERROR  - UNABLE TO GENERATE',
...
C
 ...
 961  FORMAT (/,' *****DSS*** ZRRTS:  ERROR  - NO TIME',
     * ' INTERVAL',/,' Pathname: ',A,/)
      ISTAT = 12
      GO TO 990
C
 ...
 971  FORMAT (/,' *****DSS*** ZRRTS:  ERROR  - The Data is not',
...
C
 ...
 981  FORMAT (/,' *****DSS*** ZRRTS:  ERROR  - UNABLE TO ',
...
C
 ...
 991  FORMAT(T10,'----- Exiting ZRRTS, Error return; Status:',I4)
      CALL FLUSH(MUNIT)                                                 Mu
C
      RETURN
      END
