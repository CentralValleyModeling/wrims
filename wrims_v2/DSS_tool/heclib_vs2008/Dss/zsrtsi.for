...
C     If a debug level is on, print out information
      IF (MLEVEL.GE.7) THEN
      WRITE (MUNIT,20) IFUNIT, IPLAN
 20   FORMAT (T10,'----- Enter ZSRTS for unit',I5,' -----',/,
     *        T10,'      IPLAN: ',I4)
      ENDIF
C
C
...
C     Unform the pathname
 ...
 30   CONTINUE
      ...
      IF ((NPATH.GT.MAXPATH).OR.(NPATH.LE.1)) GO TO 900
...
C
      ...
      IF (CPART(4)(1:3).EQ.'TS-') THEN
         ...
         CALL ZSRTPA (IFLTAB, CPATH, SVALUES, NVALS, CUNITS, CTYPE,
...
C     Obtain the date of the first block
      CALL ZBEGDT (JULS, INTL, IYR, IMON, IDAY, IBLOCK, IFLTAB(KVERNO))
      JULSD = IYMDJL (IYR, IMON, IDAY)
C
C     Get the date of the last block
      CALL ZBEGDT (JULE, INTL, JYR, JMON, JDAY, IBLOCK, IFLTAB(KVERNO))
...
C     Read the compressed data
      ...
      CALL ZGTREC (IFLTAB, SBUFF2, NCDA, INFO(NPPWRD+KIADAT),
     * .FALSE.)
...
C     Move data values from VALUES into BUFF
      IF (.NOT.LUPRTS) THEN
      CALL ZMOVBK (SVALUES, DVALUES, LDOUBLE, LFILDOB,
...
C
 ...
 340  FORMAT (' -----DSS---ZSRTSX;  WARNING:  Unable to Compress',
...
C	Add coordinate info  
 ...
 350	CONTINUE
      ...
      CALL ZMOVWD(COORDS(1), IIHEAD(13), NCOORDS*2)
      NDES = MIN(NCDESC, 6)
      DO 355 I=1,NDES
...
C        Data is all missing, bypass 
 ...
 380        FORMAT (' -----DSS---ZSRTS: All values set to missing',/,
     *      ' Pathname: ',A)   
         GO TO 600
      ENDIF
C
...
C     Be sure that we have permission to write this record
 ...
 400  FORMAT (' -----DSS---ZSRTSX:  Write Protection for Existing',
...
C
 ...
 410        FORMAT (' -----DSS---ZSRTS: Caution:  Writing flags to an',
...
C              record without flags to one that has flags
 ...
 420        FORMAT (' -----DSS---ZSRTS: Caution:  Writing data without',
...
C     the entire record (i.e., not changing the number of data) 
      IF (.NOT.LUPRTS) THEN
      CALL ZOWRIT (IFLTAB, CTSPAT, NTSPAT, KIHEAD, NCHEAD, NUH,
     * NTDATA)
      ENDIF
      ELSE
C     Write a new record to the DSS file
      CALL ZNWRIT (IFLTAB, CTSPAT, NTSPAT, KIHEAD, NCHEAD, NUH,
     * NTDATA)
      ENDIF
      IF (IFLTAB(KSTAT).NE.0) GO TO 961
C
...
C     Now store the header arrays
      CALL ZPTREC (IFLTAB, IIHEAD, KIHEAD, INFO(NPPWRD+KIAIHE), .FALSE.)
...
C     Only store the data passed to us (faster than zmovbk)
     ...
     * NDATA, NVALS, SVALUES, JQUAL, LQUAL, IPLAN, LERR)
      ...
      ENDIF
         IF (LDOUBLE) THEN
            IF (IFLTAB(KDSWAP).NE.0) CALL ZDSWAP(DBUFF(IPOS), NTDATA)
            CALL ZPTREC (IFLTAB, DBUFF(IPOS), NTDATA,
     *                   INFO(NPPWRD+KIADAT),.FALSE.)
         ELSE
            CALL ZPTREC (IFLTAB, BUFF(IPOS), NTDATA,
     *                   INFO(NPPWRD+KIADAT),.FALSE.)
         ENDIF
      ENDIF
C
...
C     Check to see if we should delete the record if all missing
 ...
 470           CONTINUE
            ENDIF
         ELSE
C        Data to store is all missing.  Read record to see if it is all missing
            IADD = INFO(NPPWRD+KIADAT) 
            NTDATA = INFO(NPPWRD+KINDAT)            
            IF (LDOUBLE) THEN
               CALL ZGTREC (IFLTAB, DBUFF, NTDATA, IADD, .FALSE.)
               IF (IFLTAB(KDSWAP).NE.0) CALL ZDSWAP(DBUFF, N)        
               DO 480 I=1,NLDATA
                  IF (DBUFF(I).NE.-901.0) GO TO 500
 480           CONTINUE
            ELSE
               CALL ZGTREC (IFLTAB, BUFF, NTDATA, IADD, .FALSE.)
               DO 490 I=1,NLDATA
                  IF (BUFF(I).NE.-901.0) GO TO 500
 490           CONTINUE
            ENDIF  
         ENDIF	          
         IF (MLEVEL.GE.3) WRITE (MUNIT, 380) CTSPAT(1:NTSPAT)
         CALL ZDELET (IFLTAB, CTSPAT, NTSPAT, LFOUND)
         GO TO 600
      ENDIF
C
...
C
 ...
 820  FORMAT(T10,'----- Exit ZSRTS, Number of data values ',
     * 'stored:',I7,',  Status:',I4,/)
*      CALL FLUSH(MUNIT)                                         Mu
C
      RETURN
...
C
 ...
 901  FORMAT (/,' *****DSS*** ZSRTS:  ERROR  - ILLEGAL PATHNAME',
...
C
 ...
 911  FORMAT (/,' *****DSS*** ZSRTS:  ERROR  - NON-STANDARD TIME',
...
C
 ...
 921  FORMAT (/,' *****DSS*** ZSRTS:  ERROR  - NUMBER OF VALUES',
...
C
 ...
 926  FORMAT (/,' *****DSS*** ZSRTS:  ERROR  - Insufficent',
...
C
 ...
 931  FORMAT (/,' *****DSS*** ZSRTS:  ERROR  - ILLEGAL STARTING',
...
C
 ...
 941  FORMAT (/,' *****DSS*** ZSRTS:  ERROR  - ILLEGAL STARTING',
...
C
 950  CONTINUE
     ...
     * CPATH(1:NPATH)
 951  FORMAT(/,' *****DSS*** ZSRTS:  ERROR  - UNABLE TO GENERATE',
...
C
 ...
 996  FORMAT (' -----DSS---ZSRTSX:  ERROR;  Unable to store time ',
...
C
 ...
 991  FORMAT(T10,'----- Exit ZSRTS, Error return; Status:',I4)
      GO TO 800
C
      END
