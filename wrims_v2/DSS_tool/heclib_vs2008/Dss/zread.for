...
C
C     Replaced by ZREADX
C     ZREAD is used where only small integers are stored
C     (ZREADX and ZWRITX stores large integers)
C
C
C     IPLAN is a code:
...
C
      ...
      CALL ZREADX (IFLTAB, CPATH(1:NP), IIHEAD, 2, NIHEAD, IHEAD, 0, I,
     * IHEAD, 0, JH, IDATA, 0, JD, 0, LFOUND)
      IF (IFLTAB(KSTAT).NE.0) RETURN
C
      IF (LFOUND) THEN
C
C     Was this data stored by the actual ZWRITE?
      IF (NIHEAD.EQ.2) THEN
      NH = IIHEAD(1)
      ND = IIHEAD(2)
      MH = ((NH - 1) / IWORD(8)) + 1
...
C
      ...
      CALL ZRDBUF (IFLTAB, CPATH(1:NP), IHEAD, JH, MH,
     * IDATA, JD, MD, LEND, IPLAN, LFOUND)
C
      ELSE
C     Need to read data and header as portions.  This will not
C     be done too often, so we can make a few calls to ZRDBUF.
      NHA = MIN0(NH,NHEAD)
      NDA = MIN0(ND,NDATA)
      JH = ((NHA - 1) / IWORD(8)) + 1
      JD = ((NDA - 1) / IWORD(8)) + 1
C
      IF (NHA.GT.0) THEN
      CALL ZRDBUF (IFLTAB, CPATH(1:NP), IHEAD, MH, I,
     * IDATA, 0, I, LEND, IPLAN, LFOUND)
      IF (MH.NE.JH) THEN
C     Retrieve the last small integer word of the header
      CALL ZRDBUF (IFLTAB, CPATH(1:NP), IARRAY, 1, I,
     * IDATA, 0, I, LEND, IPLAN, LFOUND)
      IHEAD(NHA) = IARRAY(1)
      ENDIF
      ENDIF
C
      IF (NDA.GT.0) THEN
      CALL ZRDBUF (IFLTAB, CPATH(1:NP), IHEAD, 0, I,
