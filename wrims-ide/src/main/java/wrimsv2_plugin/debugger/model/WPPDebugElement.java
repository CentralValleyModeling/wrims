 ...
 */
public class WPPDebugElement extends DebugElement {
	 ...
	 */
	public WPPDebugElement(IDebugTarget target) {
		super(target);
	}
	...
	/**
	 * Sends a request to the WPP interprettor, waits for and returns the reply.
	 * <p>
	 ...
	 * 		the interprettor sends the error event (for example <code>unimlpemented instruction foo</code>)
	 * 		and corresponding suspend event (for example <code>suspended event unimpinstr</code>).</li>
	 * <li><code>exit</code> - end the interpretter; reply is <code>ok</code></li>
	 ...
	 * <li><code>suspend</code> - suspend execution of the program and listen for debug comands;
	 * 		reply is <code>ok</code></li>
	...
	public String sendRequest(String request) throws DebugException {
		return getWPPDebugTarget().sendRequest(request);
	}
	
	/**
	 * Returns the debug target as a WPP target.
	 * 
	 * @return WPP debug target
	 */
