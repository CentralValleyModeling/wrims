...
C     Write the message that the data was read
 ...
 50   FORMAT(' --ZREAD:  ',A)
      ELSE
      CALL ZINQIR(IFLTAB, 'RVER', CSCRAT, IVER)
...
C     Move data from buff into values and dates
      ...
      IF (LGETDOB.EQV.LFILDOB) THEN
         CALL ZIRCPY (ILBUFF(J), ITIMES(I), DVALUES, SVALUES,
     *        IQUAL, I, LFILDOB, LGETQ, .FALSE., .FALSE., LDSWAP)
      ELSE
         CALL ZIRCPY (ILBUFF(J), ITIMES(I), DVAL, SVAL,
...
C     get the last value from the record - special case
      ...
      IF (LGETDOB.EQV.LFILDOB) THEN
         CALL ZIRCPY (ILBUFF(N), ITIMES(1), DVALUES, SVALUES,
     *        IQUAL, 1, LFILDOB, LGETQ, .FALSE., .FALSE., LDSWAP)
      ELSE
         CALL ZIRCPY (ILBUFF(N), ITIMES(1), DVAL, SVAL,
     *        IQ, 1, LFILDOB, LGETQ, .FALSE., .FALSE., LDSWAP)
...
C     if LNEXT is set read one more value
     ...
     * (.NOT.LNEXT)) GO TO 800
      ...
      IF (LGETDOB.EQV.LFILDOB) THEN
         CALL ZIRCPY (ILBUFF(J), IT, DVALUES, SVALUES,
     *        IQUAL, NVALS, LFILDOB, LGETQ, .FALSE., .FALSE., LDSWAP)
      ELSE
         CALL ZIRCPY (ILBUFF(J), IT, DVAL, SVAL,
     *               IQ, 1, LFILDOB, LGETQ, .FALSE., .FALSE., LDSWAP)
...
C     MOVE HEADER INFORMATION INTO ARGUMENTS
      ...
      ELSE IF (NIHEAD.GE.16) THEN
         ...
         IF (NIHEAD.GE.28) THEN
	      CALL ZMOVWD(IIHEAD(17), COORDS(1), 6)         
            DO 811 I=1,6
...
C
 ...
 931  FORMAT (/,' *****DSS*** ZRITS:  ERROR  - The Data is not',
...
C
 ...
 941  FORMAT (/,' *****DSS*** ZRITS:  ERROR  - UNABLE TO ',
