...
C     Store old information block
      IF(MLEVEL.GE.14)WRITE(MUNIT,*)'-ZOWRIT: Store old obsolete info'
...
C     The Pathname bin has been changed - store new copy on disk
      IF (LWBIN) THEN
      IF(MLEVEL.GE.14)WRITE(MUNIT,*)'-ZOWRIT:  new size, update bin'
      I = INT(IFLTAB(KBNSIZ))
...
C     Store the information block
      ISIZE = NPPWRD + NINFO
      IF(MLEVEL.GE.14)WRITE(MUNIT,*)'-ZOWRIT:  Store info block'
...
C     If the permanent section of the file has changed, store it
      IF (LWPERM) THEN
      NADD = 1
      IF(MLEVEL.GE.14)WRITE(MUNIT,*)'-ZOWRIT:  Update root'
...
C
 ...
 820  FORMAT (T8,'-----DSS---Debug: EXIT ZOWRIT')
      RETURN
C
C
 900  CONTINUE
