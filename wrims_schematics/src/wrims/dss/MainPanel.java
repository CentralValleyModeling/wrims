...
import javax.swing.*;
import wrims.schematic.*;

public class MainPanel extends JPanel { // implements ActionListener {  //CB added ActionListener
    ...
    static boolean STOP_DSSVIEWER_METHODS = false;

	//CB added.  A thread for calculating long-term averages for the Schematic ValueNodes.
...
//		Hashtable<String, Object> _names = null;
		...
		public void run() {
			...
			try {
				if (_selectedFilesThread != null && _selectedFilesThread.isAlive()) {
					_selectedFilesThread.join();
					if (DssViewer.DSS_DEBUG) {
						System.out.println("UpdateSchematicValuesThread joined _selectedFilesThread");
						System.out.flush();
					}
				}
//				System.out.println("Calling updateValues at " + System.currentTimeMillis());
				String date = getSchematicDate();
				if (date != null && !date.trim().equals("")) {
//No longer					if (date.indexOf("-") == -1) //CB long-term averages need time for recalculation (see below)
						_schematic.updateValues(date);
				}
			} catch (InterruptedException ie) {
				if (DssViewer.DSS_DEBUG) {
					System.out.println(
						"InterruptedException when UpdateSchematicValuesThread is attempting to join SelectedFilesThread");
					System.out.flush();
				}
			}
			_updateSchematicValuesThread = null;
//			System.out.println("_updateSchematicValuesThread duration = " + (System.currentTimeMillis() - start));
		}
	};

	//CB added.  A thread for updating non-long-term average list items in date date box
		...
		public void run() {
			...
			try {
				if (_selectedFilesThread != null && _selectedFilesThread.isAlive()) {
					_selectedFilesThread.join();
					if (DssViewer.DSS_DEBUG) {
						System.out.println("_updateSchematicDateBoxListMonthsThread joined _selectedFilesThread");
						System.out.flush();
					}
				}
//				System.out.println("Calling selected files updateDateBoxTWs at " + System.currentTimeMillis());
				_schematic.updateDateBoxTWs(true, false);
			} catch (InterruptedException ie) {
				if (DssViewer.DSS_DEBUG) {
					System.out.println(
						"InterruptedException when _updateSchematicDateBoxListMonthsThread attempting to join _selectedFilesThread");
					System.out.flush();
				}
			}
			_updateSchematicDateBoxListMonthsThread = null;
//			System.out.println("_updateSchematicDateBoxListMonthsThread duration = " + (System.currentTimeMillis() - start));
		}
	};

	//CB added.  A thread for updating long-term average list items in date date box
	...
	class UpdateSchematicDateBoxListLongTermsThread extends Thread {
		...
		public void run() {
			long start = System.currentTimeMillis();
			try {
				if (_selectedFilesLongTermAverageThread != null && _selectedFilesLongTermAverageThread.isAlive()) {
					_selectedFilesLongTermAverageThread.join();
					if (DssViewer.DSS_DEBUG) {
						System.out.println("_selectedFilesLongTermAverageThread joined _selectedFilesLongTermAverageThread");
						System.out.flush();
					}
				}
//				System.out.println("Calling long terms updateDateBoxTWs at " + System.currentTimeMillis());
				_schematic.updateDateBoxTWs(true, true);
			} catch (InterruptedException ie) {
				if (DssViewer.DSS_DEBUG) {
					System.out.println(
						"InterruptedException when _selectedFilesLongTermAverageThread attempting to join _selectedFilesLongTermAverageThread");
					System.out.flush();
				}
			}
		...
		public void run() {
			...
			try {
				if (_selectedFilesThread != null && _selectedFilesThread.isAlive()) {
					_selectedFilesThread.join();
					if (DssViewer.DSS_DEBUG) {
						System.out.println("_selectedFilesLongTermAverageThread joined _selectedFilesThread");
						System.out.flush();
					}
				}
//				System.out.println("Calling selected files calculateLongTermAverages at " + System.currentTimeMillis());
				boolean wasSuccessful = getValueViewer().calculateLongTermAverages(getSchematicTimeWindows(), _dssType, true);
//				System.out.println("Finished selected files calculateLongTermAverages call at " + System.currentTimeMillis());
			} catch (InterruptedException ie) {
				if (DssViewer.DSS_DEBUG) {
					System.out.println("InterruptedException when _selectedFilesLongTermAverageThread joining _selectedFilesThread");
					System.out.flush();
				}
			}
			_selectedFilesLongTermAverageThread = null;
//			System.out.println("_selectedFilesLongTermAverageThread duration = " + (System.currentTimeMillis() - start));
		}
...
/*		public void interrupt() {
	...
	class UnselectedFilesLongTermAveragesThread extends Thread {
		...
		public void run() {
			long start = System.currentTimeMillis();
			try {
				if (_unselectedFilesThread != null && _unselectedFilesThread.isAlive()) {
					_unselectedFilesThread.join();
					if (DssViewer.DSS_DEBUG) {
						System.out.println("_unselectedFilesLongTermAverageThread joined _unselectedFilesThread");
						System.out.flush();
					}
				}
/*				if (_selectedFilesLongTermAverageThread != null && _selectedFilesLongTermAverageThread.isAlive()) {
					_selectedFilesLongTermAverageThread.join();
					if (DssViewer.DSS_DEBUG) {
						System.out.println("_unselectedFilesLongTermAverageThread joined _selectedFilesLongTermAverageThread");
						System.out.flush();
					}
				} */
//				System.out.println("Calling unselected files calculateLongTermAverages at " + System.currentTimeMillis());
				boolean wasSuccessful = getValueViewer().calculateLongTermAverages(getSchematicTimeWindows(), _dssType, false);
			} catch (InterruptedException ie) {
				if (DssViewer.DSS_DEBUG) {
					System.out.println(
						"InterruptedException when _unselectedFilesLongTermAverageThread joining _unselectedFilesThread");
					System.out.flush();
				}
			}
			...
			_schematic.resetProgressBars();
			_schematic.setProgressVisibility(false);
//			System.out.println("_unselectedFilesLongTermAverageThread duration = " + (System.currentTimeMillis() - start));
		}
	...
	//CB added.  A thread for for loading the data in the <code>_unselectedDssFiles</code>.
	...
	class SelectedFilesThread extends Thread {
		...
		/**
		 * If the <code>_unselectedFilesDataThread</code> is running, stop it.  If the
		 * <code>_longTermAverageThread</code> is running, stop it.
		 */
		public void run() {
			long start = System.currentTimeMillis();
...
//			System.out.println("Calling selected files loadAllVariableData at " + System.currentTimeMillis());
			boolean wasSuccessful = getFilterPanel().loadAllVariableData(MainPanel.this.getAllVariables(), _dssType, true);
			_selectedFilesThread = null;
//			System.out.println("_selectedFilesThread duration = " + (System.currentTimeMillis() - start));
		}
	...
	class UnselectedFilesThread extends Thread {
		...
		public void run() {
			long start = System.currentTimeMillis();
			try {
				// CB if the _selectedFilesThread is running, this thread must wait until it is done
//				if (_selectedFilesThread != null && _selectedFilesThread.isAlive()) {  //CB changed order to selected load, selected LT calc, unselected load, unselected LT calc
//					_selectedFilesThread.join();
				if (_selectedFilesLongTermAverageThread != null && _selectedFilesLongTermAverageThread.isAlive()) {
					_selectedFilesLongTermAverageThread.join();
					if (DssViewer.DSS_DEBUG) {
//						System.out.println("_unselectedFilesThread joined _selectedFilesThread");
						System.out.println("_unselectedFilesThread joined _selectedFilesLongTermAverageThread");
						System.out.flush();
					}
				}
//				System.out.println("Calling unselected files loadAllVariableData at " + System.currentTimeMillis());
				getFilterPanel().loadAllVariableData(getAllVariables(), _dssType, false);
			} catch (InterruptedException ie) {
				if (DssViewer.DSS_DEBUG) {
					System.out.println(
...
//			System.out.println("_unselectedFilesThread duration = " + (System.currentTimeMillis() - start));
		}
     ...
     * @param dssType
     */
    public void updateSchematicValues() {  //CB TODO add String arg version IF needed
    ...
    public boolean isSchematicValuesThreadAlive() {
    	return (_updateSchematicValuesThread != null && _updateSchematicValuesThread.isAlive());
    }
    ...
    public boolean isUnselectedFilesThreadAlive() {
    	return (_unselectedFilesThread != null && _unselectedFilesThread.isAlive());
    }
    ...
    public boolean isSelectedFilesLongTermAverageThreadAlive() {
    	return (_selectedFilesLongTermAverageThread != null && _selectedFilesLongTermAverageThread.isAlive());
    }
    ...
    public boolean isSchematicDateBoxListMonthsThreadAlive() {
    	return (_updateSchematicDateBoxListMonthsThread != null && _updateSchematicDateBoxListMonthsThread.isAlive());
    }
    ...
    public boolean isSchematicDateBoxListLongTermsThreadAlive() {
    	return (_updateSchematicDateBoxListLongTermsThread != null && _updateSchematicDateBoxListLongTermsThread.isAlive());
    }
    ...
    public boolean isUnselectedFilesLongTermAverageThreadAlive() {
    	return (_unselectedFilesLongTermAverageThread != null && _unselectedFilesLongTermAverageThread.isAlive());
    }
...
/* CB tried the following, but the long runtime methods continued because references to MainPanel still existed and maybe more references existed.
    	if (_selectedFilesThread != null) {
    		if (_selectedFilesThread.isAlive()) {
    	...
    	if (_updateSchematicValuesThread != null) {
	    	if (_updateSchematicValuesThread.isAlive()) {
    	...
    	if (_unselectedFilesThread != null) {
			if (_unselectedFilesThread.isAlive()) {
    	...
    	if (_longTermAverageThread != null) {
			if (_longTermAverageThread.isAlive()) {
	    		...
	    		_longTermAverageThread = null;
	    	}
    	}
    	if (_updateSchematicDateBoxListThread != null) {
	    	if (_updateSchematicDateBoxListThread.isAlive()) {
	...
	public boolean isReadyForMonthlyWork(){
		if (!isSelectedFilesThreadAlive() && !isSchematicValuesThreadAlive()
				&& !isUnselectedFilesThreadAlive() && !isSelectedFilesLongTermAverageThreadAlive()
				&& !isSchematicDateBoxListMonthsThreadAlive() && !isUnselectedFilesLongTermAverageThreadAlive()
		...
		_checker = new Thread() {
			public void run() {
				while (!isReadyForMonthlyWork()) {
					try {
						_checker.sleep(500);
						System.out.println("Not ready to restart monthly data; sleep for half second");
					} catch (InterruptedException ie) {
						ie.printStackTrace();
					}
				}
...
//    	System.out.println("Entered startMonthlyDataWork(dssType)");
    	...
    	_updateSchematicValuesThread.start();
	   	...
	   	_selectedFilesLongTermAverageThread = new SelectedFilesLongTermAveragesThread(dssType);
		...
		//CB changed order to calculate selected files long term averages before the unselected files data loading
		//    (otherwise, the unselected files may load fasted and the progress go invisible too soon or never be visible)
//   	_selectedFilesLongTermAverageThread = new SelectedFilesLongTermAveragesThread(dssType);
//    	_selectedFilesLongTermAverageThread.start();
    	...
    	_unselectedFilesLongTermAverageThread = new UnselectedFilesLongTermAveragesThread(dssType);
...
//    	System.out.println("Exited startMonthlyDataWork(dssType)");
     ...
     * CB added.
     */
//    void loadAllVariableData(Hashtable<String, Object> names, boolean selectedFiles) {
/*    boolean loadUnSelectedVariableData() {
   		return getFilterPanel().loadAllVariableData(getAllVariables(), false);
    } */
...
//    void loadAllVariableData(Hashtable<String, Object> names, boolean selectedFiles) {
    boolean loadSelectedVariableData(int dssType) {
   		return getFilterPanel().loadAllVariableData(getAllVariables(), dssType, true);
    }
     ...
     */
//    void loadAllVariableData(Hashtable<String, Object> names, boolean selectedFiles) {
/*    boolean loadUnSelectedVariableData(int dssType) {
   		return getFilterPanel().loadAllVariableData(getAllVariables(), dssType, false);
    } */
    ...
    Hashtable<String, Object> getAllVariables() {
    	if (_schematic == null) return null;
...
//v. slow   	String[] range = _filterPanel.retrieveDateRangeFromBase();
    	...
    	int startYear = Integer.parseInt(range[0].substring(3, range[0].length()));
    	String endMonth = range[1].substring(0, 3);
    	int endYear = Integer.parseInt(range[1].substring(3, range[1].length()));
...
//			datelist.get(datelist.size() - 1).setEnabled(false);
    	...
    	while (year < endYear || (year == endYear && endIndex >= monthIndex)) {
    		...
    		if (year == endYear && months[monthIndex].equals(endMonth)) break;
    		++monthIndex;
    		monthIndex %= 12;
    ...
    void setDateBoxModel(ComboBoxModel model) {
    	...
    	_schematic.getDateBox().setRenderer(new ComboRenderer());    //CB TODO - find better place for this
    	_schematic.getDateBox().addActionListener(new ComboListener(_schematic.getDateBox()));  //CB TODO - find better place for this
    }
...
//	}
    ...
    String getSchematicDate() {
    	if (_schematic != null && _schematic.getDateBox() != null && _schematic.getDateBox().getSelectedItem() != null)
    		return ((ComboItem)_schematic.getDateBox().getSelectedItem()).toString();
    	return null;
	}
...
//    	System.out.println(_schematic.getDateBox().getModel().getSize());
    ...
    void updateSchematicDateBox(String selectedTimeWindow) {
    	//CB TODO need to check time window for validity!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    	if (!dateBoxContainsTimeWindow(selectedTimeWindow)) {
    		if (_schematic.getDateBox() != null && _schematic.getDateBox().getModel() instanceof DefaultComboBoxModel) {
    			((DefaultComboBoxModel)_schematic.getDateBox().getModel()).insertElementAt(selectedTimeWindow, 1); //0 is the blank one
    ...
    /**
     * CB added.  The Schematic date combobox model must be of type DefaultComboBoxModel.
     * @param selectedTimeWindow
     * @return
     */
    boolean dateBoxContainsTimeWindow(String tw) {
    	if (_schematic.getDateBox() == null || _schematic.getDateBox().getModel() != null) {
    		DefaultComboBoxModel model = ((DefaultComboBoxModel)_schematic.getDateBox().getModel());
    		for (int i = 0; i < model.getSize(); ++i) {
    			String timeWindow = ((ComboItem)model.getElementAt(i)).toString();
    ...
    public FilterPanel getFilterPanel() { return _filterPanel; }
    public MessagePanel getMessagePanel() { return _messagePanel; }
    ...
    protected JToolBar initToolbar() {
        ...
        JButton button = null;
        button = toolBar.add(getActions().FilterAction);
        button = toolBar.add(getActions().RetrieveAction);
        button = toolBar.add(getActions().BaseDVOpenAction);
...
//        JMenu editmenu = new JMenu();
    ...
    private JPanel createStatusPanel() {
         ...
         progressBar.setVisible(false);       //// ?? temporarily false  //CB wonder what progress Tom wanted to show here
    ...
    public void setStatus(String s) { _status.setText("Status: "+s); }
    public void setStatus() { _status.setText("Status: Done"); }
