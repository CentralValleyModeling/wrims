      SUBROUTINE ZINTBK (DATA, NSD, NED, JULS, ISTIME, INTD,
     + NDINWS, WRKSPC, NDATA, JULSWK, INTLW, OFSETW, ISTAT, IFLAG)
C
C     Interpolate block for ZTSINT
C
...
C     Update the time to prevent truncation.
      J = JULS
      I = ISTIME
      CALL ZOFSET(J,I,INTD,1,IOFS)
C
      NSWKS = NOPERS(INTD,0,JULSWK,0,J,I)
C     If an offset existed, ZOFSET upped time to nearest interval.
...
C
 ...
 25   FORMAT(T20,' --- ZINTBK CALCULATIONS --',
...
C
 ...
 100   FORMAT(' ZINTBK,II,N,WRK,TEMP=',I8,2F12.1)
C     Check for missing data.
      IF((WRKSPC(N+35).LE.-901.).AND.(WRKSPC(N+35).GE.-902.)) GO TO 140
C     Sum data.
...
C     Check for missing data values.
      IF ((RFRAC.GT..001).AND.(WRKSPC(K+35).LT.-900.).AND.
     *(WRKSPC(K+35).GT.-903.)) GO TO 320
      IF ((RFRAC.LT..99).AND.(WRKSPC(J+35).LT.-900.).AND.
     *(WRKSPC(J+35).GT.-903.)) GO TO 325
C
...
C
      ...
      DO 710 N=IBEG,IEND
C     Check for missing data.
      IF((WRKSPC(N+35).LE.-901.).AND.(WRKSPC(N+35).GE.-903.)) GO TO 740
C     Sum data.
      TEMP = TEMP + WRKSPC(N+35)
 710  CONTINUE
C
...
C
 ...
 911  FORMAT(//,' *****  ERROR - ZINTBK - STARTING DATE BAD',
     +4(2X,I6)/)
      ISTAT = 21
      RETURN
C
