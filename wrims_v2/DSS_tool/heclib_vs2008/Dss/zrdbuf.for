...
C
 ...
 20   FORMAT (T6,'-----DSS---Debug: Enter ZRDBUF',/,T10,
     * 'UNIT =',I5,'  PATH: ',A)
C
...
C
      IF (LNEW) THEN
C
      IF (MLEVEL.GE.10) WRITE (MUNIT,*)'--ZRDBUF:  New Record'
C
...
C
      ELSE
C
      IF (MLEVEL.GE.10) WRITE (MUNIT,*)'--ZRDBUF:  Second Read'
C
...
C
 ...
 820  FORMAT (T6,'-----DSS--Debug: EXIT ZRDBUF',/,T14,
     * 'FOUND = ',L1)
*      CALL FLUSH(MUNIT)                                       Mu
C
      RETURN
