...
C
C
      CHARACTER CPART(6)*64, CDASH*64, CFORMT*10
      CHARACTER CLINE*392, CLINEL*392, CSTR*400, CPATH*392, CTAG*8
...
C
 ...
 20   FORMAT (T6, '-----DSS---Debug:  Enter ZCAOUT')
C
C     Initializations.
      DO 40 I=1,6
      ILPRTL(I) = 1
...
C     Get the julian date for the next block
      ...
      IF (IST.EQ.0) THEN
C     Regular interval
      CALL ZBEGDT (JULEL, INTL, IYR, IMON, IDAY, IBLOCK, IFLTAB(KVERNO))
...
C
      IF (LBLANK(4) .AND. (JULS.EQ.JULEL) .AND. (JULEL.NE.-777777)) THEN
...
C
 ...
 820  FORMAT (T6,'-----DSS---Debug:  EXIT ZCAOUT')
      RETURN
C
C
