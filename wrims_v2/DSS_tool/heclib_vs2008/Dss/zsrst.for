     ...
     * ISBU,   ! underflow handling type for shift breakpoints for each shift
     * SBUV,   ! shift breakpoints underflow values for each shift
     * ISBO,   ! overflow handling type for shift breakpoints for each shift
     * SBOV,   ! shift breakpoints overflow values for each shift
     * ISHI,   ! interpolation type for shift heights for each shift
     * ISHU,   ! underflow handling type for shift heights for each shift
     ...
     * NSHFTP, ! number of variable shift breakpoints for each shift (0 = constant shift)
     * SHIFTS, ! shift points (for each shift, constant shift or break(1), height(1),...break(n),height(n))
       !-------------!
       ! offset info !
       !-------------!
     ...
     * OBUV,   ! offset breakpoint underflow value (may or may not be used)
     * IOBO,   ! overflow handling type for variable offset breakpoints
     * OBOV,   ! offset breakpoint oferflow value (may or may not be used)
     ...
     * OFFSET, ! variable offset values (break(1), height(1),...break(n),height(n))
       !-----------!
       ! misc info !
       !-----------!
   ...
   10 FORMAT (T5, '----- Enter ZSRST -----', /,
     ...
     * CPATH, NPATH, ISTAT)
      ...
      IF (IFLTAB(1) .NE. 6) THEN
         CALL ZERROR(IFLTAB, 5, 'ZSRST ', 0, IFLTAB, ' ', 0, ' ', 0)
      END IF
...
C         IF (NSHVAL .GT. UBOUND(SHIFTS)) GO TO 9830
         ...
         DO I = 1, NSHIFT
            ...
            DO WHILE (.NOT. LFOUND .AND. (J .LE. NEXTYP))
               IF (ISBU(I) .EQ. IEXTYP(J)) THEN
                  LFOUND = .TRUE.
                  IF (ISBU(I) .GE. 16) GO TO 9780 ! time-only extrapolation
               END IF
               J = J + 1
            END DO
            ...
            DO WHILE (.NOT. LFOUND .AND. (J .LE. NEXTYP))
               IF (ISBO(I) .EQ. IEXTYP(J)) THEN
                  LFOUND = .TRUE.
                  IF (ISBO(I) .GE. 16) GO TO 9770 ! time-only extrapolation
               END IF
               J = J + 1
            END DO
            ...
            DO WHILE (.NOT. LFOUND .AND. (J .LE. NEXTYP))
               IF (ISHU(I) .EQ. IEXTYP(J)) THEN
                  LFOUND = .TRUE.
                  IF (ISHU(I) .GE. 16) GO TO 9750 ! time-only extrapolation
               END IF
               J = J + 1
            END DO
            ...
            DO WHILE (.NOT. LFOUND .AND. (J .LE. NEXTYP))
               IF (ISHO(I) .EQ. IEXTYP(J)) THEN
                  LFOUND = .TRUE.
                  IF (ISHO(I) .GE. 16) GO TO 9740 ! time-only extrapolation
               END IF
               J = J + 1
            END DO
     ...
     *        KUHEAD, NUHEAD, ISTAT)
      ...
      !------------------!
      ISTAT = 0
      CALL ZWRITX(IFLTAB, CPATH, NPATH, IGBUFF, 16, ICHEAD, 0, IUHEAD,
...
C     * CPATH(1:NPATH)
C 9971 FORMAT (/, ' -----DSS---ZSRST:  ERROR;  The Number of Flows',
...
C     * CPATH(1:NPATH)
C 9981 FORMAT (/, ' -----DSS---ZSRST:  ERROR;  The Number of Heights',
C     * ' Supplied for the Base Curve is Less than the Coordinate',
