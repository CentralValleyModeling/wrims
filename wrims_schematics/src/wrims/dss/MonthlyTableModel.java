...
/**
 * TableModel used by the AllLayerWindow and AllProjectWindow InternalFrame classes for displaying
 * data in the GUI.
 *
 * @author Clay Booher
 */
public final class MonthlyTableModel extends AbstractTableModel { // extends TimeSeriesDataModel {
	...
	public MonthlyTableModel() {
		...
		HecTime.setAlwaysShowTimeAsBeginningOfDay(false); // not working!!!!!!!!!!!!
	}
	...
	public int setData(java.util.List dataSets, int firstMonth, boolean showCommas,
		...
		for (int i = 0; i < dataSets.size(); i++) {
			TimeSeriesContainer timeSeriesContainer = (TimeSeriesContainer) dataSets.get(i);
			//CB added hasTotalColumn code (must consider each data set separately)
			boolean hasTotalColumn;
			if ((showTotalColumn && timeSeriesContainer.units.trim().equalsIgnoreCase("CFS"))
					|| timeSeriesContainer.units.trim().equalsIgnoreCase("TAF"))
				hasTotalColumn = true;
			else
				hasTotalColumn = false;
			_dataSets.add(new SingleMonthlyTable(timeSeriesContainer, firstMonth, hasTotalColumn,
				precision));
			if (i == 0) {
		...
		if (haveData == true) return 0;
		return -1;
	}

	public int addData(java.util.List dataSets, int firstMonth, boolean showCommas,
			boolean showTotalColumn, int precision) {
		if (_dataSets == null) _dataSets = new Vector(dataSets.size());
		_showCommas = showCommas;
		_time = new HecTimeArray();
		boolean haveData = false;
		String timeZoneID = null;
		...
		for (int i = 0; i < dataSets.size(); i++) {
			...
			//CB added hasTotalColumn code (must consider each data set separately)
			boolean hasTotalColumn;
			if ((timeSeriesContainer.units.trim().equalsIgnoreCase("TAF")
					|| (showTotalColumn && timeSeriesContainer.units.trim().equalsIgnoreCase("CFS"))))
				hasTotalColumn = true;
			else
				hasTotalColumn = false;
			_dataSets.add(new SingleMonthlyTable(timeSeriesContainer, firstMonth, hasTotalColumn,
	...
	public void setShowTotalColumn(boolean showTotalColumn) {
    	...
    	for (int i = 0; i < _dataSets.size(); ++i) {
    		if (_dataSets.get(i) instanceof SingleMonthlyTable) {
    			if (((SingleMonthlyTable)_dataSets.get(i)).getModel() instanceof
    					SingleMonthlyTable.SingleMonthlyTableModel) {
    				((SingleMonthlyTable.SingleMonthlyTableModel)((SingleMonthlyTable)_dataSets
    					.get(i)).getModel()).setShowTotalColumn(showTotalColumn);
    				((SingleMonthlyTable.SingleMonthlyTableModel)((SingleMonthlyTable)_dataSets
	...
	/**
	 * Allows each set to have its own precision, but not (yet?) implemented in menuitems.
	 */
    public void setPrecision(int precision) {
    	if (_dataSets == null) return;
    	for (int i = 0; i < _dataSets.size(); ++i) {
    		if (_dataSets.get(i) instanceof TimeSeriesContainer) { // TO DO: need to remove???
    			((TimeSeriesContainer)_dataSets.get(i)).precision = precision; // TO DO:  works??
    ...
    public int getPrecision() {
    	if (_dataSets == null) return -1;
		int number = -1;
		if (_dataSets.size() > 0) { // TO DO: Determine if all sets should have the same precision
			if (_dataSets.get(0) instanceof TimeSeriesContainer) { // TO DO: need to remove???
				return ((TimeSeriesContainer)_dataSets.get(0)).precision;
			} else if (_dataSets.get(0) instanceof SingleMonthlyTable) {
    			if (((SingleMonthlyTable)_dataSets.get(0)).getModel() instanceof
    					SingleMonthlyTable.SingleMonthlyTableModel) {
    ...
    public void tabulate(JFrame parent) {
		Vector dataContainers = getDataSets();
		if (dataContainers.size() != 0) {
			TableFrame table = new TableFrame(parent); //, "", false, dataContainers);
...
/*    public String print() {
    	if (_dataSets != null) {
    		Enumeration dataSets = _dataSets.elements();
    		String printString = null;
    		while (dataSets.hasMoreElements()) {
    			Object element = dataSets.nextElement();
    			if (element instanceof SingleMonthlyTable) {
    				if (((SingleMonthlyTable)element).getModel()
    						instanceof SingleMonthlyTable.SingleMonthlyTableModel) {
    					printString = allDataString();
    				}
    			}
    		}
    		return printString;
...
//		int _defaultColumnHeaderHeight = DEFAULT_COLUMN_HEADER_HEIGHT; // TO DO:  from preferences
		int _maxNumGroupsOfAnyColumn = 0; // the max number of ColumnGroups to which any column belongs.

		public SingleMonthlyTable(TimeSeriesContainer tsc) {
			this(tsc, 0, false, 0);
		}

		public SingleMonthlyTable(TimeSeriesContainer tsc, int firstMonth, boolean showTotalColumn,
				int precision) {
			_tsc = tsc;
			setModel(new SingleMonthlyTableModel(firstMonth, showTotalColumn, precision));
			...
			currentTime.setTimeInMillis(System.currentTimeMillis(), zone.getRawOffset()/60000);
		...
		void buildHeader() {
			ColumnGroup[] columnGroups = new ColumnGroup[_tableHeaders.length + getModel().getColumnCount()];
			GroupableTableHeader header = (GroupableTableHeader)getTableHeader();
			TableColumnModel tcm = header.getColumnModel();
//			for (int i = 0; i < columnGroups.length; ++i) { // correct for 4 rows in column
			for (int i = 0; i < _tableHeaders.length; ++i) {
				columnGroups[i] = new ColumnGroup((String)_tableHeaders[i]);
				for (int j = 0; j < getColumnCount(); ++j) { // exchange inner and outer loops
					columnGroups[i].add(tcm.getColumn(j));
				}
				header.addColumnGroup(columnGroups[i]);
			}
			for (int i = 0; i < getModel().getColumnCount(); ++i) {
				columnGroups[_tableHeaders.length + i] = new ColumnGroup(getModel().getColumnName(i));
			...
			for (int i = 0; i < getColumnCount(); ++i) {
				Enumeration cgroupEnum = header.getColumnGroups(tcm.getColumn(i));
				numColumnGroupsOfColumn = 0;
				ColumnGroup columnGroup = null;
				while (cgroupEnum.hasMoreElements()) {
					Object next = cgroupEnum.nextElement();
					// TableColumn header renderers are null which cause rendereing problems (in getHeaderHeight())
					// All ColumnGroups use same renderer so need jut one ColumnGroup
...
//				tcm.getColumn(i).setCellRenderer(new SingleMonthlyTableCellRenderer());
				if (columnGroup != null)
					tcm.getColumn(i).setHeaderRenderer(columnGroup.getHeaderRenderer());
			}
		}
		...
		public void tableChanged(TableModelEvent e) {
			...
			if (getModel() instanceof SingleMonthlyTableModel) ((SingleMonthlyTableModel)getModel())
...
//				if (col.getIdentifier() instanceof String && !((String)col.getIdentifier()).equalsIgnoreCase("TTL"))
		...
		public class ColumnGroup {
			...
			public ColumnGroup(TableCellRenderer renderer, String text) {
				if (renderer == null) {
					this.renderer = new DefaultTableCellRenderer() { // Header Renderer
						public Component getTableCellRendererComponent(JTable table, Object value,
							boolean isSelected, boolean hasFocus, int row, int column) {
								JTableHeader header = table.getTableHeader();
								if (header != null) {
									setForeground(header.getForeground());
									setBackground(header.getBackground()); // gray usually
//									setForeground(table.getForeground());
//									setBackground(table.getBackground()); // white usually
		...
		public class GroupableTableHeader extends JTableHeader {
			...
			public Enumeration getColumnGroups(TableColumn col) {
				...
				Vector v_ret = null; // CB moved this outside while loop to allow return of more than one ColumnGroup
				// CB changed loop to allow return of more than one ColumnGroup for more than one header row
				while (e.hasMoreElements()) {
					ColumnGroup cGroup = (ColumnGroup) e.nextElement();
					Vector cGroup_v_ret = (Vector) cGroup.getColumnGroups(col, new Vector()); // CB altered from v_ret
					if (cGroup_v_ret.size() > 0) {
						if (v_ret == null)
							v_ret = new Vector();
						Enumeration cGroupsIn_v_ret = cGroup_v_ret.elements(); // CB added
						while (cGroupsIn_v_ret.hasMoreElements()) { // CB added
							v_ret.add(cGroupsIn_v_ret.nextElement()); // CB added
						} // CB added
					}
				}
				// CB moved this outside while loop to allow return of more than one ColumnGroup
				if (v_ret != null) {
...
// CB				int columnMargin = header.getColumnModel().getColumnMargin();
...
//					cellRect.width = aColumn.getWidth() + columnMargin; // CB - no!  it makes the header sections too wide!
			...
			private void paintCell(Graphics g, Rectangle cellRect, int columnIndex) {
				...
				TableCellRenderer renderer = aColumn.getHeaderRenderer(); //CB why??
				...
				rendererPane.add(c);
				rendererPane.paintComponent(g, c, header, cellRect.x, cellRect.y, cellRect.width,
...
//CB					Component comp = renderer.getTableCellRendererComponent(header.getTable(),
//CB						aColumn.getHeaderValue(), false, false,-1, column);
					int cHeight = 0; // = comp.getPreferredSize().height; // CB - NO!  Makes an extra header row!
			...
			private Dimension createHeaderSize(long width) {
				...
				width += headerColumnModel.getColumnMargin() * headerColumnModel.getColumnCount();
		...
		class SingleMonthlyTableModel extends AbstractTableModel {
			final String[] months = { "OCT", "NOV", "DEC", "JAN", "FEB", "MAR", "APR", "MAY", "JUN",
				"JUL", "AUG", "SEP"};
			private Vector _columnNames = new Vector(); // do not need this actually
			...
			public SingleMonthlyTableModel(int firstDisplayMonth, boolean showTotalColumn,
					int precision) {
				...
				if (firstDisplayMonth == 0) //wateryear (Oct - Sep)
					_columnNames.add("WY");
				else if (firstDisplayMonth == 5) //federal contract year (Mar - Feb)
					_columnNames.add("CY");
				else // calendar year
					_columnNames.add("YEAR");

				//read the first twelve data to determine which months are to be displayed
...
//					System.out.println("dataMonth = " + dataMonth);
					displayMonths.put(dataMonth, new Object());  // dummy value - will only check if null
				}

				if (firstDisplayMonth < 0 || firstDisplayMonth > 11) firstDisplayMonth = 0;
				for (int i = firstDisplayMonth; i < months.length + firstDisplayMonth; ++i) {
					if (displayMonths.get(i % 12) != null)
						_columnNames.add(months[i % 12]);
				}
				_columnNames.add("TTL");  // the last column is the line total/# of values on the line

				_numberHeaderRows = 4;  //CB to do: eliminate hard-coding
				int startTime = _tsc.startTime; // minutes since Dec 31 1900 24:00
...
//					int column = ((months.length - numOnFirstRow) + j) % months.length;
					int column = ((displayMonths.size() - numOnFirstRow) + j) % displayMonths.size();
					if (column == 0 || j == 0) {
...
//							System.out.println(hecTime.month());
							if (hecTime.month() > 9) //OCT, NOV, DEC
								row[0] = String.valueOf(hecTime.year() + 1); // NO row header
							else //all other months
								row[0] = String.valueOf(hecTime.year()); // NO row header
							...
							if (String.valueOf(hecTime.year()).endsWith("9")) {
								row[0] = String.valueOf(hecTime.year()) + "-"
									+ String.valueOf(hecTime.year() + 1).substring(length - 2,
											length);
							} else {
								row[0] = String.valueOf(hecTime.year()) + "-"
									+ String.valueOf(hecTime.year() + 1).substring(length - 1,
											length);
							}
						}
						if (j == 0 && column > 0) {
							for (int k = 0; k < column; ++k) {
								((HecDoubleArray)row[1]).element(k).set(-901.0); // -901 or -902????????
								((HecDoubleArray)row[1]).element(k).setUndefined();
							}
						}
					}
					if (row != null) {
						((HecDoubleArray)row[1]).element(column).set(_hdaValues.element(j)); //_hdaValues has all time window data and nothing extra
						if (column == displayMonths.size() - 1 || j == numValues - 1) {
//							if (column == displayMonths.size() - 1 ) {
								int numValid = 0;
								double sum = 0.0;
								for (int k = 0; k < displayMonths.size(); ++k) {
									if (((HecDoubleArray)row[1]).element(k).isDefined()) {
										++numValid;
										sum += ((HecDoubleArray)row[1]).element(k).value();  //CB TO DO: check (summing year and avg column???
									}
								}
								totalSum += sum;
								totalNumValid += numValid;
//								((HecDoubleArray)row[1]).element(displayMonths.size()).set(sum/numValid);  //average column
								((HecDoubleArray)row[1]).element(displayMonths.size()).set(sum); // total column
//							if (column == displayMonths.size() - 1) { //CB DOES NOT WORK FOR LESS THAN 12 MONTHS IN A ROW.
//							int subtraction = 0;
//							if (!isShowTotalColumn())
//								++subtraction;
							if (column == _columnNames.size() - 2) { // - subtraction) {
								if (j == numValues - 1) {  // THIS FIXES EXCEPT FOR LAST ROW for if (column == displayMonths.size() - 1) as predecessor line
				...
				while (dataRows.hasMoreElements()) {
					...
					if (averageColumnValues == null && ((HecDoubleArray)row[1]).numberElements() > 0) {
						averageColumnValues = new double[((HecDoubleArray)row[1]).numberElements()];
						minColumnValues = new double[((HecDoubleArray)row[1]).numberElements()];
						for (int i = 0; i < minColumnValues.length; ++i) {
							minColumnValues[i] = Integer.MAX_VALUE;
						}
						maxColumnValues = new double[((HecDoubleArray)row[1]).numberElements()];
						for (int i = 0; i < maxColumnValues.length; ++i) {
							maxColumnValues[i] = Integer.MIN_VALUE;
						}
						numColumnValues = new int[((HecDoubleArray)row[1]).numberElements()];
					}
					for (int i = 0; i < ((HecDoubleArray)row[1]).numberElements(); ++i) {
						if (((HecDoubleArray)row[1]).element(i).isDefined()) {
							averageColumnValues[i] += ((HecDoubleArray)row[1]).element(i).value();
							if (((HecDoubleArray)row[1]).element(i).value() < minColumnValues[i])
								minColumnValues[i] = ((HecDoubleArray)row[1]).element(i).value();
							if (((HecDoubleArray)row[1]).element(i).value() > maxColumnValues[i])
								maxColumnValues[i] = ((HecDoubleArray)row[1]).element(i).value();
							++numColumnValues[i];
						}
					}
				}
				for (int i = 0; i < averageColumnValues.length; ++i) {
					averageColumnValues[i] = averageColumnValues[i]/numColumnValues[i];
				}

				//replace the avgColumnValues row's avg column (last column) value with actual average, cause columns can have different number of data)
//				avgColumnValues[avgColumnValues.length - 1] = totalSum/totalNumValid;
				averageColumnValues[averageColumnValues.length - 1] = totalSum/totalNumValid;
				row = new Object[2];
				row[0] = "AVG";
			...
			/**
			 * Originally intended to allow column visibility, but only the needed data is used,
			 * so only need this method for the row average column.
			 */
			...
			void updateLowestRowValues() {
				...
				while (rows.hasMoreElements()) {
					...
					Object[] rowArray = (Object[])rows.nextElement(); // NO row header
					int subtraction = 0;
					if (!isShowTotalColumn())
						++subtraction;
					for (int j = 0; j < ((HecDoubleArray)rowArray[1]).numberElements() - 1 - subtraction; ++j) {  // 13 elements
						if (_visibleColumns[j]) {   // 14 length
							rowOfVisibles.element(j).set(((HecDoubleArray)rowArray[1]).element(j));
							if (!((HecDoubleArray)rowArray[1]).element(j).isDefined())
								rowOfVisibles.element(j).setUndefined();
						}
					}
					lowestRowValues[i++] = ((HecDoubleArray)rowOfVisibles).minimum().value();
				}
			}
			...
			void updateHighestRowValues() {
				...
				while (rows.hasMoreElements()) {
					...
					if (!isShowTotalColumn())
						++subtraction;
					for (int j = 0; j < ((HecDoubleArray)rowArray[1]).numberElements() - 1 - subtraction; ++j) {
					...
					highestRowValues[i++] =((HecDoubleArray)rowOfVisibles).maximum().value();
				}
			}

			// CB: before considering precision, I wrote this method.  Now I think I should have
			//   not used a Vector of HecDoubleArrays, but instead one HecDoubleArray (_hdaValues).
			//   Sure getValueAt would have been more complicated, but changing the precision would
			//   have been cleaner.
		    void setPrecision(int precision) {
		    	// should check to make sure position is in proper range (whatever it is)
		    ...
		    public int getPrecision() {
		    	if (_hdaValues != null) return _hdaValues.precision();
		    	else return -1;
		    }
...
//				return _columnNames.size(); // ALL columns visible
			...
			//   have been cleaner.
			public Object getValueAt(int row, int column) {
//				System.out.println("row = " + row + " and column = " + column); // debugging
/*rowheader ver				if (_data.get(row) instanceof HecDoubleArray) {
					if (((HecDoubleArray)_data.get(row)).element(getColumnNumber(column))
							instanceof HecDouble) {
						return ((HecDouble) ((HecDoubleArray) _data.get(row))
								.element(getColumnNumber(column))).string(
								((HecDoubleArray) _data.get(row)).precision(),
								_showCommas);
					} else {
						return ((Vector)_data.get(row)).get(getColumnNumber(column));
					}
				} else if (_data.get(row) instanceof String) {
					return (String)_data.get(row);
				} else {
					return null;
				} */
				...
				if (column == 0) {
					...
					return ((HecDoubleArray)rowArray[1]).element(getColumnNumber(column - 1)).string(
...
/*
			public String print() {
				...
				String line = '\t' + "BOB"; // getNominalName(); // debugging
...
*/
		...
		class SingleMonthlyTableCellRenderer extends JLabel implements TableCellRenderer {
			Font normal = null;
			Font bold = null;
			...
			public SingleMonthlyTableCellRenderer() {
				...
				if (normal == null) {
					normal = getFont();
					bold = new Font(normal.getName(), Font.BOLD,
						normal.getSize());
					boldItalic = new Font(normal.getName(), Font.BOLD + Font.ITALIC,
						normal.getSize());
				}
			}
...
//				Enumeration columns = table.getColumnModel().getColumns();
				if (column > 0) { // && column < table.getColumnModel().getColumnCount() - 1) {
					if (((HecDoubleArray)rowArray[1]).element(column - 1).value()
							== ((SingleMonthlyTableModel)table.getModel())
							.lowestRowValues[row]) {
						setForeground(Color.RED);
						setBackground(Color.WHITE);
						setFont(boldItalic);
						setHorizontalAlignment(SwingConstants.RIGHT);
					} else if (((HecDoubleArray)rowArray[1]).element(column - 1).value()
								== ((SingleMonthlyTableModel)table.getModel())
								.highestRowValues[row]) {
