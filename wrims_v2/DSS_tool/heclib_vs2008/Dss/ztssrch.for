...
C
 ...
 20   FORMAT (T6,'-----DSS---Debug:  Enter ZTSSRCH;  Unit:',I5)
C
      NFOUND = 0
      COUTPATH = ' '
C
...
C        Now check for the match
         IF ((CINPATH(1:I4TH).EQ.CPATH(1:I4TH)).AND.
     *       (CINPATH(I5TH:NP).EQ.CPATH(J5TH:NPATH))) THEN
C           Found one!
C           Return this one
            IF (IUNIT.LE.0) THEN
...
C
 ...
 820  FORMAT (T6,'-----DSS---Debug:  Exit  ZTSSRCH')
      RETURN
C
C
 900  CONTINUE
      CALL ZERROR (IFLTAB, 100, 'ZTSSRCH', 0, IFLTAB(KTABLE), ' ', 0,
     * ' ', 0)
C
 940  CONTINUE
      NFOUND = -2
