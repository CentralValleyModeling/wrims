      SUBROUTINE ZBDUMP (IFLTAB, IBGBUF)
C
...
C
 ...
 20   FORMAT (T5,'-----DSS---ZBDUMP:  Dump Buffers for unit:',I5,
...
C
      ...
      WRITE (MUNIT,*)'ZBDUMP:  Illegal Arguments',IBGBUF
      MINBUF = 1
      ELSE
      MINBUF = IBGBUF
...
C     Is there a write lock set for this buffer?
      ...
      CALL ZWMEM (JBUNIT(I), JCREC(I), IBUFF(1,I), NBSIZE, ISTAT) 
      ELSE
      CALL ZWREC  (JBUNIT(I), JCREC(I), IBUFF(1,I), NBSIZE,
...
C     Check for a write error
 ...
 120  CONTINUE
C
      IF (MLEVEL.GE.10) WRITE (MUNIT,*)'After Statement 120'
C
...
C     Check for a write error
 ...
 140  CONTINUE
C
      IF (MLEVEL.GE.10) WRITE (MUNIT,*)'After Statement 140'
C
...
C     Check for a write error
 ...
 160  CONTINUE
C
      IF (MLEVEL.GE.10) WRITE (MUNIT,*)'After Statement 160'
C
...
C     No - some other error
 ...
 910  FORMAT (///,' ********* DSS ********* ERROR ON WRITE REQUEST',
     * /,' ROUTINE ZBDUMP, UNIT, RECORD =',2I8,'  STATUS =',2I8,/,
     * '  Current Records: ',8I8)
C
      IFLTAB(1) = JCREC(I)
      CALL ZABORT (IFLTAB, 40, 'ZBDUMP', JSTAT, IFLTAB(1), ' ')
      RETURN
C
      END
