...
c      DATA MONTHS# months of the water year /'OCT','NOV','DEC','JAN','FEB','MAR',
c     1            'APR','MAY','JUN','JUL','AUG','SEP'/
      DATA MONTHS/'OCT','NOV','DEC','JAN','FEB','MAR',
     1            'APR','MAY','JUN','JUL','AUG','SEP'/
c      DATA NTYP# ? /'BP','ET','CP','SM','ST','AW','TC','HD','HN'/
...
C...        NON-CRITICAL / CRITICAL WY IRRIG SCEDULES      - KG 2/26/88
      DO K=1,NOCRP
ccc	MFINC# month when irrigation not critical (month,crop) (M,K) ,M=1,12)
         READ(4,91)(MFINC(M,K),M=1,12)
      enddo
      DO K=1,NOCRP
ccc	MFIC# month when irrigation is critical by (month, crop) (M,K),M=1,12)
         READ(4,91)(MFIC(M,K),M=1,12)
      enddo
C...        PAN EVAPORATION / ET ADJUSTMENT FACTORS        - KG 3/3/88
      DO N=1,NOCONYR

ccc	EVAP# ET adj. factor {pan evap *100/ avg evap} (month, num years from 1921) (M,N),M=1,12) 
...
ccc	STARTC# ? starting root depth (crop)
            READ(1,100)CROOT(K),STARTC(K)
         enddo

ccc	DGROOT# dry grain root depth
ccc	STRTDG# ? starting dry grain root depth
ccc	DGMOIS# ? dry grain moisture
         READ(1,100)DGROOT,STRTDG,DGMOIS

ccc	VROOT# native vegetation root depth
ccc	STARTV# ? starting native vegetation root depth
ccc	VMOIST# ? native vegetation moisture
         READ(1,100)VROOT,STARTV,VMOIST

         DO K=1,NOCRP
ccc	UMOIST# max. soil moisture (month,crop)
ccc	PMOIST# min. soil moisture (month, crop)
            READ(1,102)(UMOIST(M,K),M=1,12),(PMOIST(M,K),M=1,12)
         enddo 

         DO  K=1,NOCRP
ccc	SEEPCR# crop seepage (month, crop)
            READ(1,102)(SEEPCR(M,K),M=1,12)
         enddo
      
ccc	SEEPDG# dry grain seepage (month)
ccc	SEEPNV# native veg.seepage (month)
...
ccc	PCTNVH# ?
ccc	PCTNVP# ?
ccc	first reading at line 192 =>   1URPCT     25    34    37    19    38    47                                     
...
Ccc            IF(KDEF1(N).LT.NFYR) then !GO TO 180
            IF((1900+KDEF1(N)).LT.NFYR) then !GO TO 180
               n=n+1
...
Ccc            else IF(KDEF1(N).EQ.0.OR.KDEF1(N).GT.LYR) then !GO TO 181
            else IF(KDEF1(N).EQ.0.OR.(1900+KDEF1(N)).GT.LYR) then !GO TO 181
               n=11
            else
...
ccc		the percent of acreage for crop 'FIELD' is lowered by ~ 5% for NDEF years from 1921
...
ccc		the percent of acreage for crop 'FIELD' is lowered by ~ 11.6% for NDEF years from 1921
...
ccc	RMRVBA# ? "0"
	 READ(1,2003)RMRVBA
         ...
         DO NY=1,NOYR
ccc	RMRVAC# ? riparian veg. acreage (NY),
...
ccc	DGAC# dry grain acreage ="0" in critical years (NY)
         ...
         DO 211 L=1,NOLEV
ccc	PRVAC# ? P riparian veg. acreage "0"(L),
...
C***** test output
	IF (NY.EQ.1) THEN
 		IF (J.EQ.2) PRINT *,TOTHDP(J),RMRV2P(J),0.0
	ENDIF
...
ccc	CUS# water serface seep (month) (M),M=1,12),(
...
ccc	CUINRV# Riparian residual CU to met from inflow in inches (M),
